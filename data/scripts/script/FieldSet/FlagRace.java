...
public class FlagRace extends ScriptEngineNPC {
    ...
    public void flag_NPC() {
        ...
        int v = self.askMenu("#fs11#\r\n먼 세계에서 게임을 만들 땐 이렇게 인기가 좋지 않았는데... 나의 아름다운 เดือน드에서 플래그 레이스를 즐기고 싶은거지?\r\n\r\n이번 สัปดาห์의 코스 : #r#e" + course + "#n#k\r\n\r\n#b#L0#플래그 레이스에 도전 싶어요.#l#k\r\n#b#L1#플래그 레이스 연습 รอ실로 ย้าย시켜 สัปดาห์세요.#l#k\r\n#b#L2#플래그 레이스에 대해서 알려สัปดาห์세요.#l#k", ScriptMessageFlag.NpcReplacedByNpc);
        switch (v) {
            case 0: //플래그 레이스에 도전 싶어요.
                if (getPlayer().getGuild() == null) {
                    self.say("#fs11#\r\n플래그 레이스는 #rสัปดาห์간 กิลด์ ภารกิจ คะแนน 1 이상 ได้รับ한 자#k만이 เข้าร่วม할 수 있어. เงื่อนไข을 갖춘 뒤에 다시 시도해 줘.", ScriptMessageFlag.NpcReplacedByNpc);
                } else {
                    if (getPlayer().getGuild().getPointLogByType(GuildContentsType.WEEK_MISSIONS, getPlayer()) < 0) {
                        ...
                        else if (enter == 1) self.sayOk("#fs11#<플래그레이스> 혼자서만 도전할 수 있어.\r\nปาร์ตี้ ปลดล็อก 다시 찾아오도록 해");
                        else if (enter == 2) self.say("#fs11#เลเวล ต่ำสุด " + fieldSet.minLv + " 이상이어야 플래그레이스를 할 수 있어.");
                        else if (enter == -2)
                            self.sayOk("#fs11#플래그레이스는 매시 30นาที에 개최 돼 เวลา을 ยืนยัน 다시 시도해 줘");
                        else if (enter == -3)
                            self.sayOk("#fs11#วัน요วัน 23시 ~ เดือน요วัน 01시는 노블레스 คะแนน 정산เวลา이라 연습โหมด만 เป็นไปได้해~! เวลา을 ยืนยัน 다시 시도해 줘");
                    }
                }
                break;
            case 1: //플래그 레이스 연습 รอ실로 ย้าย시켜 สัปดาห์세요.
                if (1 == self.askYesNo("#fs11##b플래그 레이스 연습 รอ실#k 지금 ย้าย할래?", ScriptMessageFlag.NpcReplacedByNpc)) {
                    registerTransferField(942003050); //플래그 레이스 : 연습 รอ실
                }
                break;
            case 2: //플래그 레이스에 대해서 알려สัปดาห์세요.
                int vv = self.askMenu("#fs11#쿠쿠쿠, 플래그 레이스에 대해서 뭐가 그렇게 알고 싶은데?\r\n\r\n#b#L0#플래그 레이스가 뭔가요?#l\r\n#b#L1#플래그 레이스를  뭐가 좋죠?#l\r\n#L4#플래그 레이스의 랭킹 คะแนน 기준을 알고 싶어요.#l\r\n#L3#아무 것도 아니에요.#l", ScriptMessageFlag.NpcReplacedByNpc);
                switch (vv) {
                    case 0: //플래그 레이스가 뭔가요?
                        ...
                        self.say("#fs11#\r\n플래그 레이스 내에서는 오로지 ย้าย과 점프만으로 정정당당하게 승부... 하는 건 아니고, 코스 내에서만 ใช้할 수 있는 플래그 สกิล들이 있어.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n플래그 สกิล들과 แผนที่ 배치된 장치들을 잘 조합 통상의 สถานการณ์에서는 갈 수 없는 길을 가거나 더 빠르게 코스를 สัปดาห์파할 수 있을 거야.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n아, 그리고 레이스 เข้า 시 ทั้งหมด 변신 สถานะ가 ปลดล็อก 레이스 중 변신 포션을 ใช้하더라도 변신이 되지 않을 거야. 소중한 물약을 날려버릴 수도 있으니 조심하라고.", ScriptMessageFlag.NpcReplacedByNpc);
                        ...
                        break;
                    case 1: //플래그 레이스를  뭐가 좋죠?
                        self.say("#fs11#\r\n뭐가 좋으냐고? 길이 있으니까 달리고, 장애물이 있으니까 넘는 거지 꼭 좋을 게 있어야 하나?", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n...농담이고, กิลด์ สกิล 중 노블레스 สกิล 직접 투자할 수 있는 SP วันสัปดาห์วัน간의 플래그 레이스 กิลด์ 랭킹에 따라 받을 수 있지.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11##b#e1위 : 25개의 SP\r\n2위 : 22개의 SP\r\n3위 : 20개의 SP\r\n4위 ~ 10위 : 18개의 SP\r\n상위 10% : 16개의 SP\r\n상위 20% : 14개의 SP\r\n상위 30% : 12개의 SP\r\n상위 40% : 10개의 SP\r\n상위 50% : 9개의 SP\r\n상위 60% : 8개의 SP\r\n상위 70% : 7개의 SP\r\n상위 80% : 6개의 SP\r\n1,000คะแนน 이상 : 5개의 SP#n#k\r\n\r\n#r#e※ 단, 1,000คะแนน 이상 ได้รับ하지 못 순위 안에 들었더라도 รางวัล을 받을 수 없.#k#n", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n플래그 레이스 กิลด์ 랭킹은 แต่ละ กิลด์원의 플래그 레이스 점수를 합산한 점수로 매겨지지. 달리기 실력이 빼어난 동료가 많은 กิลด์ 더 높은 순위를 가져갈 수 있어.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n너의 플래그 레이스 점수는 สัปดาห์간 최고 기록으로 결정돼. 지금의 기록이 마음에 들지 않으면 언제든 다시 도전하라고. 물론 열려 있을 때 말이야.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n아, กิลด์ 탈퇴하거나 추방당 너의 สัปดาห์간 최고 기록이 วินาที기화 กิลด์ 합산 점수도 그만큼 หัก되니까 반드시 조심하라고.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n그리고 랭킹은 영원하지 않아. 매สัปดาห์ เดือน요วัน 0시에 랭킹과 노블레스 สกิล 모두 วินาที기화 그때까지 합산된 랭킹을 반영해서 노블레스 สกิลSP สัปดาห์니까. 이번 สัปดาห์에 ล้มเหลว했다고 해서 너무 실망하지 말라구.", ScriptMessageFlag.NpcReplacedByNpc);
                        break;
                    case 4: //플래그 레이스의 랭킹 คะแนน 기준을 알고 싶어요.
                        self.say("#fs11#\r\n이제 플래그 레이스에서 얻을 수 있는 랭킹 คะแนน 세 바퀴를 모두 완สัปดาห์하는데 걸린 เวลา에 따라 결정돼. 남들과 상관 없이 너만 잘 달리면 높은 점수를 얻을 수 있다는 뜻이지.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\n랭킹 คะแนน 기준은 วันสัปดาห์วัน 간 너의 최고 기록을 기준으로 결정돼. 더 높은 기록을 얻어 기록을 갱신할 수 있으니까 기록이 아쉬우면 다시 도전하라고.", ScriptMessageFlag.NpcReplacedByNpc);
                        self.say("#fs11#\r\nครั้งสุดท้าย으로, 플래그 레이스의 코스는 매สัปดาห์ เดือน요วัน 0시마다 바뀌게 돼. 코스마다 점수 기준이 다르니까 꼭 참고해서 똑똑하게 달리라고.", ScriptMessageFlag.NpcReplacedByNpc);
        ...
        switch (getPlayer().getMapId()) {
            ...
            case 942003002: {
                if (getPlayer().getOneInfoQuestInteger(32581, "mode") > 1) {
                    ...
                    int v = self.askMenu("#fs11#플래그 레이스는 재미있게 즐겼어? 무슨 วัน이야?\r\n#b#L0# 곳에서 나가고 싶어요.#l\r\n#b#L1#플래그 레이스에 대해 알려สัปดาห์세요.#l");
                    switch (v) {
                        case 1: { //플래그 레이스에 대해서 알려สัปดาห์세요.
                            int vv = self.askMenu("#fs11#쿠쿠쿠, 플래그 레이스에 대해서 뭐가 그렇게 알고 싶은데?\r\n\r\n#b#L0#플래그 레이스가 뭔가요?#l\r\n#b#L1#플래그 레이스를  뭐가 좋죠?#l\r\n#L4#플래그 레이스의 랭킹 คะแนน 기준을 알고 싶어요.#l\r\n#L3#아무 것도 아니에요.#l");
                            switch (vv) {
                                case 0: //플래그 레이스가 뭔가요?
                                    ...
                                    self.say("#fs11#\r\n플래그 레이스 내에서는 오로지 ย้าย과 점프만으로 정정당당하게 승부... 하는 건 아니고, 코스 내에서만 ใช้할 수 있는 플래그 สกิล들이 있어.");
                                    self.say("#fs11#\r\n플래그 สกิล들과 แผนที่ 배치된 장치들을 잘 조합 통상의 สถานการณ์에서는 갈 수 없는 길을 가거나 더 빠르게 코스를 สัปดาห์파할 수 있을 거야.");
                                    self.say("#fs11#\r\n아, 그리고 레이스 เข้า 시 ทั้งหมด 변신 สถานะ가 ปลดล็อก 레이스 중 변신 포션을 ใช้하더라도 변신이 되지 않을 거야. 소중한 물약을 날려버릴 수도 있으니 조심하라고.");
                                    self.say("#fs11#\r\n101เลเวล 이상만 เข้าร่วม할 수 있으니 ยืนยัน해 두라고. 이미 101เลเวล 넘은지 한참 됐다면야 뭐...");
                                    self.say("#fs11#\r\n그럼, 힘내라고.");
                                    break;
                                case 1: //플래그 레이스를  뭐가 좋죠?
                                    self.say("#fs11#\r\n뭐가 좋으냐고? 길이 있으니까 달리고, 장애물이 있으니까 넘는 거지 꼭 좋을 게 있어야 하나?");
                                    self.say("#fs11#\r\n...농담이고, กิลด์ สกิล 중 노블레스 สกิล 직접 투자할 수 있는 SP วันสัปดาห์วัน간의 플래그 레이스 กิลด์ 랭킹에 따라 받을 수 있지.");
                                    self.say("#fs11##b#e1위 : 25개의 SP\r\n2위 : 22개의 SP\r\n3위 : 20개의 SP\r\n4위 ~ 10위 : 18개의 SP\r\n상위 10% : 16개의 SP\r\n상위 20% : 14개의 SP\r\n상위 30% : 12개의 SP\r\n상위 40% : 10개의 SP\r\n상위 50% : 9개의 SP\r\n상위 60% : 8개의 SP\r\n상위 70% : 7개의 SP\r\n상위 80% : 6개의 SP\r\n1,000คะแนน 이상 : 5개의 SP#n#k\r\n\r\n#r#e※ 단, 1,000คะแนน 이상 ได้รับ하지 못 순위 안에 들었더라도 รางวัล을 받을 수 없.#k#n");
                                    self.say("#fs11#\r\n플래그 레이스 กิลด์ 랭킹은 แต่ละ กิลด์원의 플래그 레이스 점수를 합산한 점수로 매겨지지. 달리기 실력이 빼어난 동료가 많은 กิลด์ 더 높은 순위를 가져갈 수 있어.");
                                    self.say("#fs11#\r\n너의 플래그 레이스 점수는 สัปดาห์간 최고 기록으로 결정돼. 지금의 기록이 마음에 들지 않으면 언제든 다시 도전하라고. 물론 열려 있을 때 말이야.");
                                    self.say("#fs11#\r\n아, กิลด์ 탈퇴하거나 추방당 너의 สัปดาห์간 최고 기록이 วินาที기화 กิลด์ 합산 점수도 그만큼 หัก되니까 반드시 조심하라고.");
                                    self.say("#fs11#\r\n그리고 랭킹은 영원하지 않아. 매สัปดาห์ เดือน요วัน 0시에 랭킹과 노블레스 สกิล 모두 วินาที기화 그때까지 합산된 랭킹을 반영해서 노블레스 สกิลSP สัปดาห์니까. 이번 สัปดาห์에 ล้มเหลว했다고 해서 너무 실망하지 말라구.");
                                    break;
                                case 4: //플래그 레이스의 랭킹 คะแนน 기준을 알고 싶어요.
                                    self.say("#fs11#\r\n이제 플래그 레이스에서 얻을 수 있는 랭킹 คะแนน 세 바퀴를 모두 완สัปดาห์하는데 걸린 เวลา에 따라 결정돼. 남들과 상관 없이 너만 잘 달리면 높은 점수를 얻을 수 있다는 뜻이지.");
                                    self.say("#fs11#\r\n랭킹 คะแนน 기준은 วันสัปดาห์วัน 간 너의 최고 기록을 기준으로 결정돼. 더 높은 기록을 얻어 기록을 갱신할 수 있으니까 기록이 아쉬우면 다시 도전하라고.");
                                    self.say("#fs11#\r\nครั้งสุดท้าย으로, 플래그 레이스의 코스는 매สัปดาห์ เดือน요วัน 0시마다 바뀌게 돼. 코스마다 점수 기준이 다르니까 꼭 참고해서 똑똑하게 달리라고.");
                        ...
                        case 0:
                            self.say("#fs11# 곳에서 나가고 싶은 거야? 나가기 전에 옆에 있는 훌륭한 게임 머신에서 랭킹을 ยืนยัน할 수 있어. 한 번 보는 것도 괜찮을 거야.");
            ...
            case 942001000:
            case 942002000: {
                int v = self.askMenu("#fs11#\r\n여긴 플래그 레이스 게임을 하기 위해 รอ자들이 기다리는 곳이야. 너무 보채지 말라고.\r\n#b#L0#플래그 레이스에 대해서 알려สัปดาห์세요.#l\r\n#b#L1# 곳에서 나가고 싶어요.#l");
                switch (v) {
                    case 0: { //플래그 레이스에 대해서 알려สัปดาห์세요.
                        int vv = self.askMenu("#fs11#쿠쿠쿠, 플래그 레이스에 대해서 뭐가 그렇게 알고 싶은데?\r\n\r\n#b#L0#플래그 레이스가 뭔가요?#l\r\n#b#L1#플래그 레이스를  뭐가 좋죠?#l\r\n#L4#플래그 레이스의 랭킹 คะแนน 기준을 알고 싶어요.#l\r\n#L3#아무 것도 아니에요.#l");
                        switch (vv) {
                            case 0: //플래그 레이스가 뭔가요?
                                self.say("#fs11#\r\n내 위대한 창작물인 플래그 레이스에 대해 알고 싶은 거야?");
                    ...
                    case 1:
                        if (1 == self.askYesNo("#fs11#정말  곳에서 나가고 싶은거야?")) {
            ...
            default: {
                int v = self.askMenu("#fs11#\r\n여기는 플래그 레이스 연습 รอ실이야~ 궁금한 거 있으면 물어봐도 돼.\r\n#b#L1#플래그 레이스 연습 코스로 가고 싶어요.#l\r\n#b#L0#플래그 레이스에 대해서 알려สัปดาห์세요.#l");
                switch (v) {
                        ...
                        switch (vv) {
                            case 1: //한낮의 설원 연습 코스
                                if (1 == self.askAccept("#fs11##b한낮의 설원 연습 코스#k ย้าย시켜 줄까?\r\n\r\n#r※ ปาร์ตี้장만 เริ่ม할 수  ทั้งหมด ปาร์ตี้원이 연습 รอ실에 있어야만 เข้า เป็นไปได้.")) {
                                    ...
                                    if (fieldSet == null) {
                                        self.sayOk("#fs11#지금은 플래그 레이스(한낮의 설원) 이용할 수 없어.");
                                        return;
                                    }
                                    ...
                                    if (enter == -1)
                                        ...
                                        self.say("#fs11#...너는 ปาร์ตี้장이 아닌걸? ปาร์ตี้장만이 เข้า을 สมัคร할 수 있다고.");
                                    else if (enter == 3)
                                    ...
                                    else if (enter == 5) self.say("#fs11#ปาร์ตี้원이 모두 모여 있어야 เริ่ม할 수 있.");
                                    else if (enter == 6)
                                        self.say("#fs11#이미 다른 ปาร์ตี้ 안으로 들어가 เควส 클리어에 도전 있는 중이랍니다.");
                                } else {
                                    self.sayOk("#fs11#연습이 จำเป็น해지면 언제든 말해...");
                                }
                                break;
                            case 2: //석양의 설원 연습 코스
                                if (1 == self.askAccept("#fs11##b석양의 설원 연습 코스#k ย้าย시켜 줄까?\r\n\r\n#r※ ปาร์ตี้장만 เริ่ม할 수  ทั้งหมด ปาร์ตี้원이 연습 รอ실에 있어야만 เข้า เป็นไปได้.")) {
                                    ...
                                    if (fieldSet == null) {
                                        self.sayOk("#fs11#지금은 플래그 레이스(석양의 설원) 이용할 수 없어.");
                                        return;
                                    }
    ...
    public void flag_Exit_NPC() {
        if (1 == self.askYesNo("#fs11#\r\n벌써 포기 싶은 거야? 밖으로 내보내줄까?", ScriptMessageFlag.NpcReplacedByNpc)) {
        ...
        switch (getPlayer().getMapId()) {
            case 942000500: { //실전แผนที่
                ...
                if (flagGoal == 3) {
                    ...
                    getPlayer().getMap().broadcastMessage(CWvsContext.getScriptProgressMessage(getPlayer().getName() + "님이 골인 지점을 통과하였. 앞으로 " + (3 - flagGoal) + "바퀴 남았."));
        ...
        switch (getPlayer().getMapId()) {
            ...
            case 942002500: {
                ...
                if (flagGoal == 3) {
                    ...
                    self.say("#b" + mapName + "#k 완สัปดาห์에 สำเร็จ했구나! 생แต่ละ보단 꽤 하는걸?", ScriptMessageFlag.NpcReplacedByNpc);
                    if (1 == self.askYesNo("내가 직접 코스 밖으로 แนะนำ해줄게. 물론 원한다면 아직도 달리고 있는 동료들을 구경해도 . 쿠쿠쿠.", ScriptMessageFlag.NpcReplacedByNpc)) {
            ...
            case 942003100: {
                ...
                int flagGoal = fieldSet.getFlagGoalNumber(getPlayer().getName());
                if (flagGoal == 3) {
                    self.say("#b한낮의 설원#k 완สัปดาห์에 สำเร็จ했구나! 생แต่ละ보단 꽤 하는걸?", ScriptMessageFlag.NpcReplacedByNpc);
                    if (1 == self.askYesNo("내가 직접 코스 밖으로 แนะนำ해줄게. 물론 원한다면 아직도 달리고 있는 동료들을 구경해도 . 쿠쿠쿠.", ScriptMessageFlag.NpcReplacedByNpc)) {
                        registerTransferField(942003000); //한낮 ออกแผนที่
                    }
                }
            ...
            case 942003200: {
                ...
                int flagGoal = fieldSet.getFlagGoalNumber(getPlayer().getName());
                if (flagGoal == 3) {
                    self.say("#b석양의 설원#k 완สัปดาห์에 สำเร็จ했구나! 생แต่ละ보단 꽤 하는걸?", ScriptMessageFlag.NpcReplacedByNpc);
                    if (1 == self.askYesNo("내가 직접 코스 밖으로 แนะนำ해줄게. 물론 원한다면 아직도 달리고 있는 동료들을 구경해도 . 쿠쿠쿠.", ScriptMessageFlag.NpcReplacedByNpc)) {
            ...
            case 942003300: {
                ...
                int flagGoal = fieldSet.getFlagGoalNumber(getPlayer().getName());
                if (flagGoal == 3) {
                    self.say("#b한밤의 설원#k 완สัปดาห์에 สำเร็จ했구나! 생แต่ละ보단 꽤 하는걸?", ScriptMessageFlag.NpcReplacedByNpc);
                    if (1 == self.askYesNo("내가 직접 코스 밖으로 แนะนำ해줄게. 물론 원한다면 아직도 달리고 있는 동료들을 구경해도 . 쿠쿠쿠.", ScriptMessageFlag.NpcReplacedByNpc)) {
