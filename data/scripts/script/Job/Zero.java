...
public class Zero extends ScriptEngineNPC {
    ...
    public void zero_inheritance() {
        ...
        if (curLv != 10) {
            self.sayReplacedNpc("이제부터 우리를 더 성장 시키기 위해서는 특별한 재료가 จำเป็น해.", 2400009, 0, ScriptMessageFlag.Scenario);
        ...
        if (getPlayer().getOneInfoQuestInteger(40981, "save_genesis") == 1) {
            if (curLv != 10) {
                ...
                v0 = self.askMenuReplacedNpc("자, #h0#. 어떻게 하겠어?\r\n\r\n#b#L3# ก่อนหน้า 단계의 라피스와 라즐리를 장착한다.#l\r\n#L999# 그만둔다.#l", 2400009, 0,  ScriptMessageFlag.NpcReplacedByNpc);
            }
        } else if (curLv == 7) {
            if (level >= 200) {
        ...
        if (v0 == 0) {
            self.sayReplacedNpc("성장하기 위해서는 #t4310216# 1개 จำเป็น해.", 2400009, 0, ScriptMessageFlag.Scenario);
            getPlayer().openInheritanceUpgrade(8, 4310216, 1);
        } else if (v0 == 1) {
            self.sayReplacedNpc("성장하기 위해서는 #t4310217# 1개 จำเป็น해.", 2400009, 0, ScriptMessageFlag.Scenario);
            getPlayer().openInheritanceUpgrade(9, 4310217, 1);
        } else if (v0 == 2) {
            self.sayReplacedNpc("제네시스 อาวุธ를 얻기 위해서는 #t4310260# 1개 จำเป็น해.", 2400009, 0, ScriptMessageFlag.Scenario);
            if (!getPlayer().haveItem(4310260)) {
                ...
                self.sayReplacedNpc("참고로 #t4310260# ใช้한 성장은 지금까지와는 다른 점이 많아!", 2400009, 0, ScriptMessageFlag.Scenario);
                self.sayReplacedNpc("#b제네시스 อาวุธ#k ก่อนหน้า 단계의 อาวุธ에서 잠재ความสามารถ, 에디셔널 잠재ความสามารถ, เพิ่มตัวเลือก 제외는 전수되지 않고 특별한 ตัวเลือก 가지고 있어.\r\n#r- สัปดาห์문의 흔적 15% โจมตี력(힘) สัปดาห์문서로 อัพเกรด เป็นไปได้ 횟수 모두 ใช้\r\n- 스타포스 22성\r\n- Unique 잠재ความสามารถ 및 Epic 에디셔널 잠재ความสามารถ มี(전수하지 않을 경우)\r\n-장착 시 <파괴의 얄다바오트>, <창조의 아이온> สกิล ได้รับ\r\n- สัปดาห์문서 및 스타포스 เสริมแรง 불가", 2400009, 0, ScriptMessageFlag.Scenario);
                self.sayReplacedNpc("또 제네시스 อาวุธ를 완성한 후에는 อาวุธ 성장 버튼으로 ก่อนหน้า 단계와 제네시스 อาวุธ를 오갈 수 있으니 제네시스 อาวุธ로 เสริมแรง하는 것을 고민하지 않아도 될거야.\r\n단, 교체하는데 힘이 มาก จำเป็น해서 한번 교체한 후 10นาที 동안은 교체할 수 없어.", 2400009, 0, ScriptMessageFlag.Scenario);
                ...
                self.sayReplacedNpc("검은 마법사의 힘이 담긴 #b#t4310260##k 구하다니!\r\n대단하구나 너!", 2400009, 0, ScriptMessageFlag.Scenario);
                self.sayReplacedNpc("#b#t4310260##k 이용 우리를 #b제네시스 อาวุธ#k เสริมแรง할 수 있어!\r\n하지만 지금까지의 เสริมแรง와는 다른 점이 많아!", 2400009, 0, ScriptMessageFlag.Scenario);
                self.sayReplacedNpc("#b제네시스 อาวุธ#k ก่อนหน้า 단계의 อาวุธ에서 잠재ความสามารถ, 에디셔널 잠재ความสามารถ, เพิ่มตัวเลือก 제외는 전수되지 않고 특별한 ตัวเลือก 가지고 있어.\r\n#r- สัปดาห์문의 흔적 15% โจมตี력(힘) สัปดาห์문서로 อัพเกรด เป็นไปได้ 횟수 모두 ใช้\r\n- 스타포스 22성\r\n- Unique 잠재ความสามารถ 및 Epic 에디셔널 잠재ความสามารถ มี(전수하지 않을 경우)\r\n-장착 시 <파괴의 얄다바오트>, <창조의 아이온> สกิล ได้รับ\r\n- สัปดาห์문서 및 스타포스 เสริมแรง 불가", 2400009, 0, ScriptMessageFlag.Scenario);
                self.sayReplacedNpc("또 제네시스 อาวุธ를 완성한 후에는 อาวุธ 성장 버튼으로 ก่อนหน้า 단계와 제네시스 อาวุธ를 오갈 수 있으니 제네시스 อาวุธ로 เสริมแรง하는 것을 고민하지 않아도 될거야.\r\n단, 교체하는데 힘이 มาก จำเป็น해서 한번 교체한 후 10นาที 동안은 교체할 수 없어.", 2400009, 0, ScriptMessageFlag.Scenario);
                int v1 = self.askMenuReplacedNpc("#r잠재ความสามารถ#k 경우 네가 원한다면 สุ่ม한 #bUnique 잠재ความสามารถ#k 변화할거야. #bUnique 잠재ความสามารถ#k 변화하겠어?\r\n\r\n#b#L0#Unique 잠재ความสามารถ으로 เปลี่ยน한다.#l\r\n#L1#기존 잠재ความสามารถ을 그대로 유지한다.#l", 2400009, 0, ScriptMessageFlag.NpcReplacedByNpc);
                int v2 = self.askMenuReplacedNpc("#r에디셔널 잠재ความสามารถ#k 경우 네가 원한다면 สุ่ม한 #bEpic 에디셔널 잠재 ตัวเลือก#k 변화할거야. #bEpic 에디셔널 잠재ความสามารถ#k 변화하겠어?\r\n\r\n#b#L0#Epic 에디셔널 잠재ความสามารถ으로 เปลี่ยน한다.#l\r\n#L1#기존 에디셔널 잠재ความสามารถ을 그대로 유지한다.#l", 2400009, 0, ScriptMessageFlag.NpcReplacedByNpc);
                self.sayReplacedNpc("#bเพิ่มตัวเลือก#k 그대로 전수되니 걱정하지마!", 2400009, 0, ScriptMessageFlag.Scenario);
                String v4 = "자! 이제 정말 ครั้งสุดท้าย이야!\r\n#t4310260# ใช้해 #b제네시스 라피스와 라즐리#k 얻겠어?\r\n";
            ...
            if (check) {
                ...
                if (alphaBefore == null || betaBefore == null) {
                    self.sayReplacedNpc("알 수 없는 오류가 발생했어. 고객센터에 문의해สัปดาห์겠어?", 2400009, 0, ScriptMessageFlag.NpcReplacedByNpc);
                    return;
                }
                ...
                self.sayReplacedNpc("ก่อนหน้า อาวุธ로 돌아가는데 많은 힘이 จำเป็น해. 아직 เวลา이 더 จำเป็น하니 나중에 다시 시도해줄래?", 2400009, 0, ScriptMessageFlag.NpcReplacedByNpc);
            }
        }
    }
    ...
    public void zero_reinvoke_weapon() {
        ...
        self.sayReplacedNpc("#face2#우왓, 이거 완전히 가루가 버렸잖아? 너, 대체 나한테 무슨 짓을 한 거야? 이제 어떡하냐고? 어떡하긴 뭘 어떡해? อาวุธ가 깨졌으니 ทั้งหมด 게 끝, GAME OVER지! 그동안 즐거웠어. 그럼 안녕……", 2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
        self.sayReplacedNpc("…라고  서운하겠지? 하하, 놀랐어? 하지만 나도 이번엔 มาก 놀랐다고. 나, 알고 보면 예민한 남자야. 소중하게 다뤄줘~", 2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
        self.sayReplacedNpc("어차피 อาวุธ는 우리를 담고 있는 그릇วัน 뿐, 우리가 그대로 남아있다면 อาวุธ는 언제든지 되살릴 수 있어. 자자, 그럼 เวลา의 힘을 모아서 이렇게……", 2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
        ...
        self.sayReplacedNpc("쨘! 어때. 감쪽같지? 응? 언제든지 재생이 เป็นไปได้하니까 마음놓고 เสริมแรง를 해도 되겠다고? 그건…", 2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
        self.sayReplacedNpc("#face2#…어떻게 그런 생แต่ละ을 할 수 있어?! อาวุธ가 깨질 때 우리도 만만찮게 충격을 받는단 말야! อาวุธ가 깨지는 바람에 얼MP 놀랐는데……", 2400010, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
        self.sayReplacedNpc("그건 라즐리 말이 맞아. 게다가 너무 강력한 ไอเท็ม เวลา의 힘으로도 되살리는데에 한계가 있어. 앞으로는 조심해!", 2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
    }

    public void zero_egoequiptalk() {
        int r = Randomizer.rand(1, 34);
        if (r == 1) {
            self.sayReplacedNpc("내가 제วัน 좋아하는 거…? 으응… 나는 딸기 케이크가 제วัน 좋아. 나랑 같이 먹고 싶다면... 응, 너라면 같이 먹어도 좋아.",  2400010, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("라피스에 대해서 어떻게 생แต่ละ하냐고? 음… 덩치만 커다란 바보?",  2400010, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("알파를 เลือก한 이유? 그건… 벼, 별다른 이유 없어. 그냥 아무나 골랐을 뿐이라구. 혹시 네가 마음에 들어서 그렇다는 착แต่ละ은 하지 말아줬으면 좋겠어!",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 5) {
            self.sayReplacedNpc("#face1#나는 밤이 싫어. 혼자 있는 것만 같은 기นาที이 든단 말야. 그래도 이젠 혼자가 아니니까 괜찮아.",  2400010, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("#face0#딱히 네가 좋아서 같이 있어สัปดาห์는 건 아니야. 그냥 나는 내 의무를 다 있는 것 뿐이니까 신경쓰지 말아줬음 좋겠어.",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 7) {
            self.sayReplacedNpc("알파에 대해서 어떻게 생แต่ละ하냐고? 어, 어떻게 생แต่ละ하긴 뭘 어떻게 생แต่ละ해? 아, 알파 같은 건 นิดหน่อย도 좋아하지 않는다구!",  2400010, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("#face0#내가 화난 것 같다고? 그냥 날씨가 별로라서 그래. 굳이 네가 나에게 오랫동안 말을 걸어สัปดาห์지 않아서 그런 게 아니라구!",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 9) {
            self.sayReplacedNpc("내가 제วัน 좋아하는 건… 당연히 고기지! 몸에 좋고 맛도 좋은 고기!",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("라즐리에 대해서 어떻게 생แต่ละ하냐고? 라즐리는… 정말 무서워. 화내면 마녀같다고. 봐, 지금도… 히익!",  2400009, 0, ScriptMessageFlag.Scenario);
        } else if (r == 11) {
            self.sayReplacedNpc("베타를 เลือก한 이유? 그야 당연히… 예쁘니까! 이왕 สัปดาห์인으로 모실 거면 예쁜 여자가 좋잖아? 하하.",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("나 제법 쓸만한 것 같지 않아? …아니라고 생แต่ละ 어쩔 수 ไม่มี.",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("그럼 기นาที이 좋을 땐?",  2400006, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
            self.sayReplacedNpc("기นาที 좋을 때도 먹는 게 최고지. 와구와구!",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("무슨 생แต่ละ을  있냐고? 사실 나는 지금 아สัปดาห์ 심แต่ละ하게… …아무 생แต่ละ도 없는데?",  2400009, 0, ScriptMessageFlag.Scenario);
        } else if (r == 16) {
            self.sayReplacedNpc("다들 조용하니까 나라도 นาที위기를 살려야지. 나는 นาที위기 메이커로써 나름의 사명감을 가지고 있다고! 단순히 심심해서 장난치는 게 아니야. …정말로!",  2400009, 0, ScriptMessageFlag.Scenario);
        } else if (r == 17) {
            self.sayReplacedNpc("บอส มอนสเตอร์ 잡으면 더 많은 wp ได้รับ할 수 있어. 물론 잡아야 ได้รับ할 수 있지, 잡아야…",  2400009, 0, ScriptMessageFlag.Scenario);
            self.sayReplacedNpc("#face0#1절만 하라고, 1절만! ",  2400005, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
        } else if (r == 18) {
            self.sayReplacedNpc("네가 성장해야 우리도 진화할 수 있어. 그러니까 몸에 좋고 맛도 좋은 고기를 มาก 먹어야… 아, 이게 아닌가?",  2400009, 0, ScriptMessageFlag.Scenario);
        } else if (r == 19) {
            self.sayReplacedNpc("สัปดาห์문서나 큐브를 ใช้ 대검과 태도에 동시에 ใช้งาน된다고. 이게 바로 1+1!",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("110, 120, 130, 140, 150, 170, 180เลเวล  อาวุธ ระดับ ขึ้น시킬 수 있어. 물론 네가 원하지 않는다면 나중에 เลือก해도 좋아.",  2400009, 0, ScriptMessageFlag.Scenario);
            self.sayReplacedNpc("뭔가 예전과 미묘하게 다른 것 같은데?",  2400010, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
            self.sayReplacedNpc("너야말로 แนะนำ를 유도하는 말투인 것 같은데?",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("그러다가 안 좋은 잠재ความสามารถ이 나오면 어떡해?",  2400010, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
            ...
            self.sayReplacedNpc("WEAPON UI에서는 อาวุธ만 เสริมแรง할 수 있어. 여긴 우리만의 공간이니까. 나머지 ป้องกัน구나 액세서리는 원래처럼 อุปกรณ์창에서 เสริมแรง하라구. ",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 25) {
            self.sayReplacedNpc("태그를 이용 좀 더 효율적인 전투가 เป็นไปได้해. 구, 굳이 나도 너희와 함께 싸우고 싶어서 그러는 건 아니니까 오해하진 말아줘.",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 26) {
            self.sayReplacedNpc("럭키ไอเท็ม สัปดาห์문서를 ใช้ 세트ไอเท็ม เอฟเฟกต์를 낼 수 있다던데. 사실 난 어디에 소속되는 걸 그리 좋아하진 않지만 말야.",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 27) {
            ...
            self.sayReplacedNpc("골드라벨 나이트 ป้องกัน구 세트라고 알아? 거울세계 บอส들이 레시피를 가지고 있다던데 구해서 만들어 보는 게 어때? 그럴 리는 없지만 의외로 너한테 어울릴지도 모르니까, 흥.",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 29) {
            ...
            self.sayReplacedNpc("우리는 당연히 두손อาวุธ지. 두손으로 공손히 모시라는 뜻이야.",  2400010, 0, ScriptMessageFlag.Scenario);
            self.sayReplacedNpc("#face7#두손อาวุธ가 그런 의미였어?!",  2400005, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
        } else if (r == 31) {
            self.sayReplacedNpc("여신의 눈물을 모으면 새로운 สกิล 배울 수 있어. …하지만 난 걔네랑 별로 같이 다니고 싶진 않단 말야.",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 32) {
            self.sayReplacedNpc("너희, 닮긴 했지만 그래도 มาก 다른걸. 헤어나 성형은 같이 쓸 수도 있지만 한 명씩도 ใช้ เป็นไปได้하니까 좀 더 개성을 추구하는 것도 나쁘지 않을지도?",  2400010, 0, ScriptMessageFlag.Scenario);
        } else if (r == 33) {
            self.sayReplacedNpc("#face1#엥? 라즐리랑 사이가 나쁘냐고? 아니, 우리 제법 친한 사이라고 생แต่ละ하는데. 안 그래?",  2400009, 0, ScriptMessageFlag.Scenario);
            ...
            self.sayReplacedNpc("#face1#근데, wp 거울세계에서만 얻을 수 있다고 내가 말했었나?",  2400009, 0, ScriptMessageFlag.Scenario);
            self.sayReplacedNpc("#face7#뭐? 거울세계에서 자유로워져도 얻을 수 없는거야?",  2400005, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
            self.sayReplacedNpc("wp 거울세계에 흩어진 เวลา의 힘. 거울세계에서만 얻을 수 있는게 당연한 거 아니야? 메이플 เดือน드에서는 군단장급의 บอส들만 wp 가지고 있어.",  2400010, 0, ScriptMessageFlag.Scenario);
            self.sayReplacedNpc("그리고 아쉽게도 고기에는 wp 함유(?) 있지 않아.",  2400009, 0, ScriptMessageFlag.Scenario, ScriptMessageFlag.FlipImage);
