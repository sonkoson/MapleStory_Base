...
public class Will extends ScriptEngineNPC {
    ...
    public void will_enterGate() {
        String Message = "윌을 저지하기 위해 #b회절의 회랑#k으로 이동할까?\r\n\r\n";
        if (DBConfig.isGanglim) {
            Message += "#L0#회절의 회랑(#b노멀 모드#k)으로 이동한다. #r(레벨 235이상) #g[" + getPlayer().getOneInfoQuestInteger(1234570, "will_clear") + "/" + (getPlayer().getBossTier() + 1) + "]#k#l\r\n";
            Message += "#L1#회절의 회랑(#b하드 모드#k)으로 이동한다. #r(레벨 235이상) #g[" + getPlayer().getOneInfoQuestInteger(1234569, "will_clear") + "/" + (getPlayer().getBossTier() + 1) + "]#k#l\r\n";
            Message += "#L2#회절의 회랑(#b노멀 연습 모드#k)으로 이동한다. #r(레벨 235이상)#k#l\r\n";
            Message += "#L3#회절의 회랑(#b하드 연습 모드#k)으로 이동한다. #r(레벨 235이상)#k#l\r\n";
            Message += "#L7#회절의 회랑(#r헬 모드#k)으로 이동한다. #r(레벨 270이상)#k#l\r\n";
            Message += "#L4#이동하지 않는다.#l\r\n\r\n";
        }
        else {
        	boolean single = getPlayer().getPartyMemberSize() == 1;
            Message += "#L5##b회절의 회랑(이지 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L0##b회절의 회랑(노멀 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L1##b회절의 회랑(하드 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L6##b회절의 회랑(이지 연습 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L2##b회절의 회랑(노멀 연습 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L3##b회절의 회랑(하드 연습 모드)" + (single ? "(싱글)" : "(멀티)") + "으로 이동한다.#r(레벨 235이상)#k#l\r\n";
            Message += "#L7##b회절의 회랑(#r헬 모드#b)으로 이동한다.#r(레벨 270이상)#k#l\r\n";
        ...
        switch (Menu) { //따로 제한되는거 없으면 바로 입장가능함
            ...
            case 1: { //하드모드
                ...
                if (enter == -3) {
                    self.sayOk("최근 일주일 이내 윌을 쓰러뜨린 파티원이 있습니다. 윌은 노멀 모드, 하드 모드, 헬 모드를 합쳐 일주일에 1회만 클리어 가능합니다\r\n#r<클리어 기록은 매주 목요일에 일괄 초기화됩니다.>", ScriptMessageFlag.Scenario, ScriptMessageFlag.NpcReplacedByNpc);
                    return;
                }
                if ((enter == -1 || enter == 4)) {
                    self.sayOk("입장 제한횟수가 부족하거나 레벨 제한이 맞지 않는 파티원이 있어 입장할 수 없습니다.", ScriptMessageFlag.Scenario, ScriptMessageFlag.NpcReplacedByNpc);
            ...
            case 7: { //헬모드
                ...
                if (enter == 3) {
                    self.sayOk("윌(헬모드)은 1~3인 구성 파티로만 도전할 수 있습니다.", ScriptMessageFlag.Scenario, ScriptMessageFlag.NpcReplacedByNpc);
                    return;
                }
                break;
            }
