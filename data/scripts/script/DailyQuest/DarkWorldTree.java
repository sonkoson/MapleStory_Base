...
public class DarkWorldTree extends ScriptEngineNPC {
    ...
    public void q39002s() {
        ...
        askText.append("이곳 세계 #b최강의 전사#k여. 이번 สัปดาห์에 도울 วัน은 아래와 같다.\r\n\r\n");
        for (Integer q : quests) {
            askText.append("#b#e#y")
                    .append(q)
                    .append("##k#n\r\n");
        }
        askText.append("\r\n#e지금 바로 타락한 세계수의 정화를 돕지 않겠나?#n\r\n(마음에 들지 않는다면 교체하기 버튼을 눌러 다른 ภารกิจ로 교체할 수 있다.)");
        if (1 == self.askYesNo(askText.toString(), ScriptMessageFlag.Change)) {
            askText = new StringBuilder();
            askText.append("รายการ에 있는 ภารกิจ가 마음에 들지 않는다면 다른 ภารกิจ를 찾아볼 수도 있지. 교체 싶은 ภารกิจ를 เลือก해라.\r\n\r\n");
            int index = 0;
            for (Integer q : quests) {
                askText.append("#b#e")
                        .append("#L")
            ...
            askText.append("#L5# #r#e더 이상 교체 싶은 ภารกิจ는 없다.#k#n#l");
            ...
            String scriptText = "제외된 ภารกิจ 대신 새로운 ภารกิจ를 찾았다.\r\n\r\n";
            ...
            self.sayOk("ภารกิจ가 모두 끝나면 내게 돌아와 เสร็จสมบูรณ์ 된다. \r\nทั้งหมด ภารกิจ는 #e#rวัน요วัน 자정#k#n까지 유효하니 내게서 รางวัล을 받아 가려면 그전에 돌아오도록 해라.");
        }
    }

    @SuppressWarnings("deprecation")
    public void q15708s() {
        self.say("혹시 วันสัปดาห์วัน 안에 세계수를 #b5회 이상 정화#k 해준다면, 자네의 ความสามารถ을 인정 #r보답#k #i4001868:# #b#t4001868##k 더 สัปดาห์겠네.", ScriptMessageFlag.NpcReplacedByNpc);
        if (self.askYesNo("나의 부하들이 저지르고 있는 만행을 멈추고 세계수의 타락을 막는 데에 힘써สัปดาห์겠나?", ScriptMessageFlag.NpcReplacedByNpc) == 1) {
            //เดือน요วัน날 리셋된다.
            Date time = new Date();
            if (time.getDay() == 1) {
                time.setDate(time.getDate() + 7);
    ...
    public void q15708e() {
        self.say("자네는 내 예상보다 훨씬 더 훌륭하게 ภารกิจ를 수행해สัปดาห์었군.", ScriptMessageFlag.NpcReplacedByNpc);
        self.say("감사의 แสดง라고 하기에는 약소하지만 #r보답#k 선물을 นิดหน่อย 더 สัปดาห์겠네.", ScriptMessageFlag.NpcReplacedByNpc);

        if (target.exchange(4001868, (2 * 2)) > 0) {
            getQuest().forceComplete(getPlayer(), getNpc().getId());
            self.say("여기 #i4001868:# #b#t4001868##k 받게. 덕นาที에 내 부하들이 악에서 헤어나올 수 있었다. 정말 고맙다...", ScriptMessageFlag.NpcReplacedByNpc, ScriptMessageFlag.NoEsc);
        } else {
            self.say("자네 가방이 가득찬거 같구만. อื่นๆ창을 1칸이상 비우고 다시와สัปดาห์게.");
        }
    }
    ...
    public void slimeQuestEnd() {
        ...
        if (target.exchange(4001868, (2 * eventRate)) > 0) {
            ...
            getQuest().forceComplete(getPlayer(), getNpc().getId());
            self.sayOk("자, 이건 #b약속한 선물#k이네.\r\n#i4001868:# #b#t4001868##k\r\n우리 종족이 뿌린 #r악의 씨앗#k 거두는 วัน을 도와줘서 고맙다..", ScriptMessageFlag.NpcReplacedByNpc);
        } else {
            self.sayOk("자네 가방이 가득찬거 같구만. อื่นๆ창을 1칸이상 비우고 다시와สัปดาห์게.");
        }
    }
