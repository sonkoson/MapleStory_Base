    ...
    while i < len(content):
        ...
        if b != -1 and (b >= 0xC0):
            ...
            if i + needed < len(content):
                ...
                # Capture segment for debug
                segment = [c]
                
                chars_consumed = 1
                for k in range(needed):
                    next_c = content[i + 1 + k]
                    segment.append(next_c)
                ...
                if valid_seq:
                    try:
                        ...
                        if debug and "เ" in segment: 
                            print(f"Decoded segment: {segment} -> {decoded}")
                        res.append(decoded)
                        i += chars_consumed
                        changes += 1
                        continue
...
def fix_file(filepath):
    ...
    if "NPCConversationManager" in filepath:
        ...
        # Print chars around corrupted area
        idx = content.find("buffGuild")
        if idx != -1:
             start = idx + 400
             end = idx + 600
             print(f"Hex dump around 1869 (approx {start}-{end}):")
             for k in range(start, end):
                 if k < len(content):
                     c = content[k]
                     print(f"offset {k}: char '{c}' U+{ord(c):04X}")
