...
var item1 = [2436603, "#z2436603#", "1"];
var ac1 = [4310228, "[Z] 새해 코인", "5"];
var ac2 = [4310237, "사냥 코인", "1"];
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			select = selection;
			if (select == 1) {
				var suk1 = Math.floor((cm.itemQuantity(2436603) / 1));
			...
			if (select == 1) {
				if (cm.haveItem(item1[0], item1[2] * cost) && cm.canHold(4310228, cost*5)) { // 새해 코인 *5
					...
					cm.sendOk("#fn나눔고딕##r#i2436603##z2436603#이 부족합니다. \r\n 또는 인벤토리가 꽉 차있는지 확인 해주시길 바랍니다");
					cm.dispose();
				}
			}
			if (select == 2) {
				if (cm.haveItem(item1[0], item1[2] * cost) && cm.canHold(4310237, cost)) { // 사냥 코인 * 1 
					if (cm.getPlayer().getInventory(Packages.objects.item.MapleInventoryType.USE).getNumFreeSlot() >= 1) {
