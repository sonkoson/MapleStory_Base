...
function action(mode, type, sel) {
	...
	if (status == 0) {
		...
		allstat = Packages.objects.utils.Randomizer.rand(normal_allstat_min, normal_allstat_max);
		atk = Packages.objects.utils.Randomizer.rand(normal_atk_min, normal_atk_max);
		dmg = Packages.objects.utils.Randomizer.rand(normal_dmg_min, normal_dmg_max);

		itemz = Packages.objects.item.MapleItemInformationProvider.getInstance().getEquipById(selected);
