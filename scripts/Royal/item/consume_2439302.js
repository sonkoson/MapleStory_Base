...
function action(mode, type, selection) {
 ...
 if (status == 0) {
	...
	if(selection == 1) {
		...
		cm.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.openUI(1207));
		cm.dispose();
	} else if(selection == 2) {
		cm.getClient().getSession().writeAndFlush(Packages.tools.packet.ContentsPacket.NoPlayerExpInfo(cm.getPlayer()));
		cm.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.openUI(1208));
		cm.dispose();
	}
	} else {
