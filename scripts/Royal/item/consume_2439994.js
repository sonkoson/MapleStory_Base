...
var items = [
  ...
  [[1672079,1], 0.8], // Limited Ring II 0.8%
  [[1182286,1], 0.8], // Limited Pendant II 0.8%
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getClient().getKeyValue("2439994ticket") == null) {
            cm.getClient().setKeyValue("2439994ticket", 0);
        }
        ...
        chat += "Limited Ticket 으로 뽑기에 도전해보세요!" + enter + enter
        chat += 별 + 색 + " 꼭 좋은 아이템을 획득하세요 ! " + 별 + enter + enter + 끝
        chat += "#L1#" + 색 + "[Limited Ticket 돌림판]" + 끝 + " 뽑기#l"
        chat += "\r\n"
        chat += "#L99#" + 색 + "[Limited Ticket 돌림판]" + 끝 + " 리스트#l"
        cm.sendSimple(chat);
    } else if (status == 1) {
        switch (selection) {
            case 1:
                if (!cm.haveItem(2439994, 1)) {
                    cm.sendOkS("#i2439994# #z2439994# 아이템이 없는것같은데?..", 700);
                    cm.dispose();
                    return;
                }
                ...
                for(var x = 0;x< amount;x++) {
                    ...
                    for(var i = 0;i<items.length;i++) {
                        ...
                        if(percentage > chance) {
                            ...
                            if (items[i][0][0] == 1672079 || items[i][0][0] == 1182286) {
                                ...
                                Center.Broadcast.broadcastGachaponMessage(cm.getPlayer().getName() + " 님이 Limited Ticket 에서 {" + cm.getItemName(items[i][0][0]) + "} 아이템을 획득하셨습니다 !", 5068300, it);
                                cm.getClient().setKeyValue(items[i][0][0], "1");
                            }
                            currentcount = cm.getClient().getKeyValue("2439994ticket");
                            cm.getClient().setKeyValue("2439994ticket", Integer.parseInt(currentcount) + pluscount);
                            break;
                        }
                    }
                }
...
function reload() {
    check1 = cm.getClient().getKeyValue("1672079");
    check2 = cm.getClient().getKeyValue("1182286");
    if (check1 != null) {
        items = [
          //[[ItemCode, Qty], Probability]
          [[1182286,1], 1.6], // Limited Pendant II
    ...
    if (check1 != null && check2 != null) {
        ...
        cm.getClient().removeKeyValue("1182286");
        items = [
          //[[ItemCode, Qty], Probability]
          [[1672079,1], 0.8], // Limited Ring II 0.8%
          [[1182286,1], 0.8], // Limited Pendant II 0.8%
