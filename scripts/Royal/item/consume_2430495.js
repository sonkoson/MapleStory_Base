...
function action(mode, type, selection) {
	...
	var leftslot1 = cm.getPlayer().getInventory(Packages.objects.item.MapleInventoryType.SETUP).getNumFreeSlot();
        if (leftslot1 < 5) {
            cm.sendOk("인벤토리 공간이 최소한 5칸은 필요합니다. 설치 탭의 공간을 5칸이상 비워주신 후 다시 열어주세요.");
            cm.dispose();
            return;
        }
	var leftslot3 = cm.getPlayer().getInventory(Packages.objects.item.MapleInventoryType.EQUIP).getNumFreeSlot();
        if (leftslot3 < 5) {
        ...
        for (var i = 0; i < 2 + Packages.objects.utils.Randomizer.nextInt(3); i++) {
            ...
            if (rand < 10) {
            ...
            } else if (rand < 30 && Packages.objects.utils.Randomizer.isSuccess(50)) {
                ...
                var meso = 0;
                if (Packages.objects.utils.RandomizerisSuccess(98)) {
                    meso = Packages.objects.utils.Randomizer.rand(10000000, 50000000);
                } else if (Packages.objects.utils.RandomizerisSuccess(50)) {
                    meso = Packages.objects.utils.Randomizer.rand(50000000, 150000000);
                } else if (Packages.objects.utils.RandomizerisSuccess(50)) {
                    meso = Packages.objects.utils.Randomizer.rand(300000000, 500000000);
                } else {
                    meso = Packages.objects.utils.Randomizer.rand(10000000, 333300000);
                }
                ...
                if (leaf[calculate][1] == 0) {
                    quantity = Packages.objects.utils.Randomizer.rand(2, 10);
                } else if (leaf[calculate][1] == 1) {
                    quantity = Packages.objects.utils.Randomizer.rand(10, 50);
                } else if (leaf[calculate][1] == 2) {
                    quantity = Packages.objects.utils.Randomizer.rand(80, 300);
                } else if (leaf[calculate][1] == 3) {
                    quantity = Packages.objects.utils.Randomizer.rand(300, 500);
                } else if (leaf[calculate][1] == 4) {
                    quantity = Packages.objects.utils.Randomizer.rand(500, 1000);
                }
