...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getClient().getKeyValue("2439993ticket") == null) {
            cm.getClient().setKeyValue("2439993ticket", 0);
        }
        ...
        switch (selection) {
            case 1:
                if (!cm.haveItem(2439993, 1)) {
                    cm.sendOkS("#i2439993# #z2439993# 아이템이 없는것같은데?..", 700);
                    cm.dispose();
                    return;
                }
                ...
                for(var x = 0;x< amount;x++) {
                    ...
                    for(var i = 0;i<items.length;i++) {
                        ...
                        if(percentage > chance) {
                            ...
                            if (items[i][0][0] == 1119501 || items[i][0][0] == 1122501) {
                            ...
                            currentcount = cm.getClient().getKeyValue("2439993ticket");
                            cm.getClient().setKeyValue("2439993ticket", Integer.parseInt(currentcount) + pluscount);
                            break;
                        }
                    }
                }
...
function reload() {
    check1 = cm.getClient().getKeyValue("1119501");
