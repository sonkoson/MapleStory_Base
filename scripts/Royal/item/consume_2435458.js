...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#b#i"+need+"##z"+need+"# "+qty+"개#k가 있으면 #b#i5060048##z5060048##k 하나랑 교환할 수 있습니다."+enter;
		...
		if (sel == 1) {
			...
			if (item == 1114316) {
			    it = Packages.objects.item.MapleItemInformationProvider.getInstance().getEquipById(item);
			    Packages.objects.item.MapleInventoryManipulator.addFromDrop(cm.getClient(), it, false, false, false, true);
			} else {
			    cm.gainItem(item, 1);
			}
