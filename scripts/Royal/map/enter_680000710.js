...
function action(mode, type, sel) {
	...
	if (status == 0) {
		...
		if (cm.getPlayer().getOneInfoQuestInteger(1234567, "see_intro") == 1) {
			...
			cm.effectText("Royal Castle ที่เต็มไปด้วยความสุขเสมอ", 100, 1000, 4, 0, 0, -200);
			cm.dispose();
		...
		cm.effectText("#fnNanumGothic ExtraBold##fs18#<Royal Castle>", 100, 1000, 6, 0, -50, -50);
		cm.delay(2000);
	} else if (status == 6) {
		...
		cm.sayReplacedNpc("เฮ้! รอนานแล้ว!\r\n#fs30##rยินดีที่ได้เจอ! ดีใจใช่ไหม? ยินดีต้อนรับ!", false, true, 1, 9062474, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
	} else if (status == 16) {
		...
		cm.sayReplacedNpc("หึหึ ขอให้ #rRoyal Castle#k เป็นความทรงจำที่ดีของคุณเหมือนกันนะ", false, true, 1, 9062474, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
	} else if (status == 54) {
		cm.sayReplacedNpc("มาสนุกกับช่วงเวลาดีๆ ที่ #rRoyal Castle#k กับพวกเราเถอะค่ะ!", false, true, 1, 9062475, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
	} else if (status == 55) {
		cm.setBlind(1, 255, 0, 0, 0, 250, 2);
		cm.delay(250);
	} else if (status == 56) {
