...
function action(mode, type, sel) {
	...
	cm.effectText("#fn나눔고딕 ExtraBold##fs18#<네오 캐슬>", 100, 1000, 6, 0, -50, -50);
	cm.delay(2000);
    } else if (status == 6) {
	...
	cm.sayReplacedNpc("헤헤! 기다렸어요!..#fs30##r반가워요! 반갑죠? 환영해요!", false, true, 1, 9062474, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
    } else if (status == 16) {
	...
	cm.sayReplacedNpc("후후. #r네오 캐슬#k이 부디 너에게도 좋은 추억으로 남길 바랄게.", false, true, 1, 9062474, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
    } else if (status == 54) {
	cm.sayReplacedNpc("저희와 함께 #r네오 캐슬#k에서 좋은 시간 보내요!", false, true, 1, 9062475, ScriptMessageFlag.Scenario, ScriptMessageFlag.NoEsc);
    } else if (status == 55) {
	cm.setBlind(1, 255, 0, 0, 0, 250, 2);
	cm.delay(250);
    } else if (status == 56) {
