...
function enter(pi) {
	...
	if (avail == null) {
		...
		pi.getClient().getSession().write(MaplePacketCreator.serverNotice(6, "ประตูนี้ถูกปิดอยู่"));
		return false;
	}
	else {
		pi.getPlayer().changeMap(target, targetPortal);
		pi.getClient().getSession().write(MaplePacketCreator.serverNotice(6, "วิ่งไปทางขวาเร็วเข้า!!!"));
		return true;
	}
}