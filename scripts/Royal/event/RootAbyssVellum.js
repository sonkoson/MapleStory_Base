...
function setup(eim) {
    ...
    while (em.getInstance("RootAbyssVellum_" + a) != null) {
        a = Packages.objects.utils.Randomizer.nextInt();
    }
    var eim = em.newInstance("RootAbyssVellum_" + a);
...
function changedMap(eim, player, mapid) {
    if (mapid != 105200400 && mapid != 105200410 && mapid != 105200800 && mapid != 105200810) {
        eim.unregisterPlayer(player);
        if (eim != null) {
            if (eim.getPlayerCount() <= 0) {
                var mode = eim.getProperty("mode");
