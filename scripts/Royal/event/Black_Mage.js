...
function playerEntry(eim, player) {
    ...
    player.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.SetIngameDirectionMode(false, false, false));
    player.getClient().getSession().writeAndFlush(CField.blind(1, 0xff, 0x0, 0x0, 0x0, 0, 0));
    player.getClient().getSession().writeAndFlush(CField.musicChange("Bgm00.img/Silence"));
    player.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.InGameDirectionEvent("", 1, 100));
...
function showEffectPhase1(eim) {
    ...
    while (iter.hasNext()) {
        ...
        player.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.InGameDirectionEvent("", 1, 5000));
...
function blackOut(eim) {
    ...
    while (iter.hasNext()) {
        ...
        player.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.InGameDirectionEvent("", 1, 1000));
    }
}
...
function removeblackOut(eim) {
    ...
    while (iter.hasNext()) {
        ...
        player.getClient().getSession().writeAndFlush(Packages.network.models.CField.UIPacket.InGameDirectionEvent("", 1, 700));
    }
}
...
function spawnMonster(eim, instance, mobid, x, y) {
    ...
    map.broadcastMessage(Packages.network.models.CField.UIPacket.openUI(1204));
}
...
function messagePlayer(eim, instance, msg) {
    var map = eim.getMapInstance(instance);
    map.broadcastMessage(CField.addPopupSay(0, 3000, msg, ""));
}
