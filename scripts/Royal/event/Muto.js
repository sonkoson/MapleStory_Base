...
function effect(eim) {
	...
	while (iter.hasNext()) {
		...
		player.getClient().getSession().writeAndFlush(Packages.network.models.CField.mutoSetTime(1000 * 60 * 10));
...
function recipe(eim) {
	...
	mutoRecipe = Math.floor(Math.random() * 7); // Only basic difficulty appears


	if (eim.getProperty("enhance").equals("true") && parseInt(eim.getProperty("score")) <= 340) {
		map.killAllMonster(eim.getPlayers().get(0)); // Hack/Workaround
		eim.setProperty("enhance", "false");
		spawnOrigin(eim);
	...
	if (eim.getProperty("enhance").equals("false") && parseInt(eim.getProperty("score")) >= 670) {
		...
		map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMutoMsg/msg2", 16));
	}
	...
	if (eim.getProperty("mutoRecipe") != null) {
		...
		if (eim.getProperty("complete") != null && eim.getProperty("complete").equals("1")) {
			...
			map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMuto/perfect", 16));
			map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMutoMsg/msg5", 16));
			if (eim.getProperty("bonus").equals("true")) {
				eim.setProperty("score", "" + (parseInt(eim.getProperty("score")) + 150));
			}
		} else {
			map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMuto/failed", 16));
			map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMutoMsg/msg6", 16));
		}
	}
	...
	map.broadcastMessage(Packages.network.models.CField.setMutoNewRecipe(recipes[mutoRecipe], rep.length, eim));
	setTimer(eim);
}
...
function monsterValue(eim, mobId) {
	...
	if (mobId == 9833261 || mobId == 9833046 || mobId == 9833066) {
		map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMutoMsg/msg7", 16));
		Packages.server.Timer.MapTimer.getInstance().schedule(function () {
			for (i = 0; i < mobidz.length; ++i) {
				if (mobidz[i] == mobId) {
					...
					map.broadcastMessage(Packages.network.models.CField.environmentChange("Map/Effect3.img/hungryMutoMsg/msg8", 16));
					break;
				}
			}
		}, 3000);
...
function clear(eim) { // clear
	...
	while (iter.hasNext()) {
		player = iter.next();
		player.changeMap(450002024, 0);
		player.getClient().getSession().writeAndFlush(Packages.network.models.CField.finishMuto());
