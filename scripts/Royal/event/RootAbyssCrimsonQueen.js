...
function setup(eim) {
    ...
    while (em.getInstance("RootAbyssCrimsonQueen_" + a) != null) {
        a = Packages.objects.utils.Randomizer.nextInt();
    }
    var eim = em.newInstance("RootAbyssCrimsonQueen_" + a);
...
function changedMap(eim, player, mapid) {
    if (mapid != 105200300 && mapid != 105200310 && mapid != 105200700 && mapid != 105200710) {
        eim.unregisterPlayer(player);
        if (eim != null) {
            if (eim.getPlayerCount() <= 0) {
                var mode = eim.getProperty("mode");
