	...
	if (re[m1].getState() == 1 && re[m2].getState() == 1 && re[m3].getState() == 1 && re[m4].getState() == 1 && re[m5].getState() == 1 && re[m6].getState() == 1) {
		...
		schedule = EtcTimer.getInstance().register(function () {
			if (tick == 5) {
				...
				rm.getMap().broadcastMessage(CField.startMapEffect("Moon Bunny จะตำแป้งต๊อกได้ก็ต่อเมื่อไม่ถูกโจมตีเท่านั้น! ช่วยรวบรวม Moon Bunny's Rice Cake ให้ครบ 80 ชิ้นทีนะ!", 5120016, true));
			}
			tick++;
		}, 1000)
	}
