...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        ...
        if (Packages.objects.utils.Randomizer.rand(1, 100) <= item['chance']) {
            ...
            cm.worldGMMessage(21, "[혼돈아케인] " + cm.getPlayer().getName() + "님께서 [" + cm.getItemName(item['itemid']) + "] 아이템 제작에 성공하였습니다!");
        } else {
            cm.sendOk("#fs11#제작에 실패했습니다.");
            cm.worldGMMessage(21, "[혼돈아케인] " + cm.getPlayer().getName() + "님께서 [" + cm.getItemName(item['itemid']) + "] 아이템 제작에 실패하였습니다.");
            gainFail(item);
        }
        cm.dispose();
    }
