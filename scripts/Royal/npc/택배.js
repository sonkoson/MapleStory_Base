    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (timecheck <= (1000 * 60 * 60 * 24 * 2)) {
                cm.sendOk("#fs11#계정 생성 후 3일이 지난 계정만 이용할 수 있습니다.");
                cm.dispose();
                return;
            }
            ...
            if (operation == 1) {
                ...
                var msg = 별파 + "#fs11##fc0xFF000000# 현재 보낼 수 있는 아이템 리스트입니다! #fs11#" + 별파 + 엔터 + 엔터;
                msg += 색 + "#e< 큐브 >#n\r\n";
            ...
            if (selection >= 1 && selection <= 6 || selection == 55) {
                cm.sendGetText("#fs11#" + 검은색 + "택배받으실 분의 닉네임을 입력해주세요.#r\r\n\r\n받으시는분은 같은 채널에 접속중이여야 합니다.\r\n고유 아이템은 택배시 이미 아이템을 보유 중이면 증발되며 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않습니다,");
            } else if (selection == 7) {
                ...
                for (var x = 1; x < 2; x++) {
                    ...
                    for (var i = 0; i <= cm.getInventory(yes).getSlotLimit(); i++) {
                        ...
                        if (operation == 55) {
                            ...
                            selStr += 검은색 + "#L" + (yes * 1000 + i) + "##i" + itemid + "##z" + itemid + "##l\r\n";
                        }
                    }
                }
                if (!ok) {
