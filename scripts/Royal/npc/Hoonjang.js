...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        var msg = "#fs11#" + enter;
        msg += "#fc0xFF000000#ดูเหมือนจะมี Medal หลากหลายแบบเตรียมไว้#b" + enter;
        msg += "สุ่ม - #i" + need + "##z" + need + "# " + qty + " ชิ้น" + enter;
        //msg += "เลือก - #i"+selectNeed+"##z"+selectNeed+"# "+selectQty+" items#b"+enter;
        msg += "#L1##dตรวจสอบรายการไอเท็ม" + enter;
        msg += "#L2#ลองสุ่มดูสิ\r\n";
        //msg += "#L3#เลือกแบบที่ต้องการ";
        ...
        switch (sel) {
            ...
            case 2:
                if (!cm.haveItem(need, qty)) {
                    cm.sendOk("#fs11#วัสดุที่ต้องใช้ในการสุ่มไม่เพียงพอ");
                    cm.dispose();
                    return;
                }
                ...
                if (!cm.canHold(dskin[a], 1)) {
                    cm.sendOk("#fs11#ช่องเก็บของไม่เพียงพอ หรือเป็นไอเท็มเฉพาะ (Unique Item)");
                    cm.dispose();
                    return;
                }
                ...
                break;
            case 3:
                var msg = "#fs11#เลือก Medal ที่ต้องการสิ #fs11##b"
                for (i = 0; i < dskin.length; i++) {
                    if (i % range == 0) {
                        msg += "\r\n"
                    }
