/*
Selectable Special Cash - Chaewon
*/
black = "#fc0xFF191919#"
...
/*for (var cap = 1008000; cap < 1009000; cap++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(cap) != null) {
        if (MapleItemInformationProvider.getInstance().getName(cap).contains("[Zenia]")) {
            itemsz0.push(cap);
        }
    }
}
...
for (var coat = 1042800; coat < 1042900; coat++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(coat) != null) {
        if (MapleItemInformationProvider.getInstance().getName(coat).contains("[Zenia]")) {
            itemsz1.push(coat);
        }
    }
}
...
for (var pants = 1062800; pants < 1062900; pants++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(pants) != null) {
        if (MapleItemInformationProvider.getInstance().getName(pants).contains("[Zenia]")) {
            itemsz2.push(pants);
        }
    }
}
...
for (var longcoat = 1055000; longcoat < 1057000; longcoat++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(longcoat) != null) {
        if (MapleItemInformationProvider.getInstance().getName(longcoat).contains("[Zenia]")) {
...
for (var glove = 1088000; glove < 1089000; glove++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(glove) != null) {
        if (MapleItemInformationProvider.getInstance().getName(glove).contains("[Zenia]")) {
            itemsz4.push(glove);
        }
    }
}
...
for (var shose = 1078000; shose < 1079000; shose++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(shose) != null) {
        if (MapleItemInformationProvider.getInstance().getName(shose).contains("[Zenia]")) {
            itemsz5.push(shose);
        }
    }
}
...
for (var cape = 1104000; cape < 1105000; cape++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(cape) != null) {
        if (MapleItemInformationProvider.getInstance().getName(cape).contains("[Zenia]")) {
            itemsz6.push(cape);
        }
    }
}
...
for (var weapon = 1703500; weapon < 1703800; weapon++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(weapon) != null) {
        if (MapleItemInformationProvider.getInstance().getName(weapon).contains("[Zenia]")) {
            itemsz7.push(weapon);
        }
    }
}
...
for (var Accessory = 1012900; Accessory < 1012990; Accessory++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(Accessory) != null) {
        if (MapleItemInformationProvider.getInstance().getName(Accessory).contains("[Zenia]")) {
...
for (var glasses = 1022800; glasses < 1022890; glasses++) {
    if (MapleItemInformationProvider.getInstance().getItemInformation(glasses) != null) {
        if (MapleItemInformationProvider.getInstance().getName(glasses).contains("[Zenia]")) {
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (check == 0) {
            ...
            msg = "'" + cm.getPlayer().getName() + "' is suspicious.\r\n";
            msg = "'" + a.getFullYear() + "-" + Number(a.getMonth() + 1) + "-" + a.getDate() + " " + a.getHours() + ":" + a.getMinutes() + ":" + a.getSeconds() + "'\r\n";
            msg += "Script attempted copy: ColorChangeCash\r\n";
            msg += "User Character ID: " + cm.getPlayer().getId() + "\r\n";
            msg += "User Account ID: " + cm.getPlayer().getAccountID() + "\r\n";
            out1.write(msg.getBytes());
            out1.close();
            cm.getPlayer().getWorldGMMsg(cm.getPlayer(), "Attempted copy in ColorChangeCash.");
            cm.sendOk(itemsArray[select] + "#fs11#การเข้าถึงผิดปกติ", 9062004);
            cm.dispose();
            return;
        }
        var itemsArray = getArray(menuSelect);
        talk = "#fs11#" + black + "คุณต้องการรับไอเท็มด้านล่างนี้ใช่หรือไม่?\r\n"
        talk += "#fc0xFFD5D5D5#───────────────────────────#k\r\n\r\n";
        talk += "#i" + itemsArray[select] + "##b#z" + itemsArray[select] + "##k"
        cm.sendYesNo(talk);
    } else if (status == 3) {
        var itemsArray = getArray(menuSelect);
        if (cm.canHold(itemsArray[select])) {
            ...
            talk = "#fs11#" + black + "เป็นอย่างไรบ้าง? ถูกใจไหม? เรามีชุดหลากหลายเตรียมไว้ให้ แวะมาอีกนะ~!\r\n\r\n"
            talk += "#fUI/UIWindow2.img/QuestIcon/4/0#\r\n"
            talk += "#i" + itemsArray[select] + "##b#z" + itemsArray[select] + "##k"
            cm.sendOk(talk);
            cm.dispose();
        } else {
