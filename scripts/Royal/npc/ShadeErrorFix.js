...
function action(mode, type, sel) {
    ...
    if (!cm.getPlayer().isGM() && cm.getPlayer().getDonationPoint() < price && status > 1 && seld != 2) {
        cm.sendOk("#fs11##fc0xFFFF3300#" + price + "P#fc0xFF000000# Royal Points are required.");
        cm.dispose();
        return;
    }
    ...
    if (status == 0) {
        if (cm.getPlayer().getJob() != 2512) {
            cm.sendOk("#fs11#Only Shade can use this.");
            cm.dispose();
            return;
        }
        if (!cm.getPlayer().vCoreEquipCheck()) {
            cm.sendOk("#fs11#Please unequip all V Matrix Cores before trying.");
            cm.dispose();
            return;
        }
        cm.dispose();
        ...
        if (deamon == 1) {
            cm.sendOk("#fs11#Job change complete.\r\n#r※ MP gauge will be generated after changing channels and reconnecting.");
        } else {
            cm.effectText("#fnNanumGothic ExtraBold##fs20#[Error Fix] < " + tempjob['jobname'] + " > error has been fixed.", 50, 1000, 6, 0, 330, -550);
        }
        prevflag = cm.getPlayer().getSaveFlag();
        cm.getPlayer().setSaveFlag(1024); // VMatrix
        cm.getPlayer().saveToDB(false, false);
        cm.getPlayer().setSaveFlag(prevflag);
        cm.addCustomLog(20, "[Job Change] [Shade Error Fix]");
    }
}
