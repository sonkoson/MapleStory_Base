...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.getPlayer().getKeyValue("Quest_" + quest[sel][1]) == Today) {
            cm.sendOk("#fs11#이 퀘스트는 이미 완료된 퀘스트 입니다.");
            cm.dispose();
            return;
        ...
        if (bClear) {
            ...
            cm.sendYesNo("#fs11#해당 퀘스트를 완료하기 위한 조건이 충족되었습니다. 퀘스트를 완료하시겠습니까?\r\n\r\n" + say);
        } else {
        ...
        for (var i = 0; i < reward.length; i++) {
            if (!cm.canHold(reward[i][0], reward[i][1])) {
                cm.sendOk("#fs11#인벤토리 슬롯이 부족하거나 꽉 찬 것은 아닌지 확인해 주세요.");
                return;
            }
        }
        ...
        cm.sendOk("#fs11#퀘스트가 완료되어 보상이 지급되었습니다. 인벤토리를 확인해 주세요.");
