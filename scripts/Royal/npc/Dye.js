...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        codyList = [];
        gColor = !cm.getPlayer().getGender() ? "#fs11##dFemale" : "#fs11##bMale";
        seld = selection;
        selStr = "";
        ...
        switch (seld) {
            ...
            case 2:
                ...
                break;
            case 3:
                selStr += "#fnNanumGothic ExtraBold##fs11##e#r[Character COLOR]#b#n\r\n";
                selStr += "#L8# #eRegular Dye#n\r\n#l";
                //     selStr += "#L21# #eMix Dye#n\r\n#l";
                //selStr += "#L7# #eChange Lens Color#n\r\n#l";
                //   selStr += "#L9# #eChange Skin Color#n#l#k\r\n ";
                /*  if (cm.getPlayer().getAndroid() != null) {
                      selStr += "\r\n\r\n#e#r[Android COLOR]#b#n\r\n";
                      selStr += "#L19# #b#eAndroid#n#b Regular Dye#n\r\n#l";
                      selStr += "#L18# #b#eAndroid#n#b Change Lens Color#n\r\n#l";
                      selStr += "#L20# #b#eAndroid#n#b Change Skin Color#n#l#b\r\n";
                  }*/
                break;
            case 4:
                selStr += "#fnNanumGothic ExtraBold##fs11#I will change #h0#'s gender.\r\n";
                selStr += "#L10# Change gender to #e" + gColor + "#n#l#b\r\n";
                break;
            case 5:
                cm.dispose();
                cm.openShop(1022002);
            ...
            case 6:
                ...
                break;
            case 7:
                cm.sendOk("#fnNanumGothic ExtraBold##fs11#Coming soon..");
                cm.dispose();
                return;
                break;
        }
        ...
        if (selection == 8 || selection == 21) {
            for (i = 0; i < bannedhair.length; i++) {
                if (cm.getPlayer().getHair() == bannedhair[i]) {
                    cm.sendOk("This hair cannot be dyed.");
                    cm.dispose();
                    return;
                }
            }
        ...
        if (SEL_00 == 10) {
            if (cm.getPlayer().getJob() == 10112) {
                cm.sendOkS("Zero job cannot change gender.", 4);
                cm.dispose();
                return;
            }
            cm.sendYesNoS("Do you want to change gender to #e" + gColor + "#n#k? You will no longer be able to wear equipment for your current gender.", 4);
        }

        Beauty = SEL_00 < 2 ? "#ehair#n" : SEL_00 == 2 ? "#eface#n" : "#ecolor#n";
        selStr = "#fnDotum##fc0xFFFFFFFF#I can change your current " + Beauty + " to a completely new style. If you are bored with your current look, take your time to choose the hair you want.";
        ...
        switch (SEL_00) {
            ...
            case 18: // Hair 2
                for (i = 0; i < 8; i++) {
                    codyList.push(Math.floor((SEL_00 >= 12 ? cm.getPlayer().getAndroid().getFace() : Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer().getJob()) ? cm.getPlayer().isDressUp() ? cm.getPlayer().getSecondFace() : cm.getPlayer().getFace() : cm.getPlayer().getFace()) / 1000) * 1000 + ((SEL_00 >= 12 ? cm.getPlayer().getAndroid().getFace() : Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer().getJob()) ? cm.getPlayer().isDressUp() ? cm.getPlayer().getSecondFace() : cm.getPlayer().getFace() : cm.getPlayer().getFace()) % 100) + i * 100);
                }
                break;
            case 8:
            case 19: // Hair 2
                for (i = 0; i < 8; i++) {
                    codyList.push(Math.floor((SEL_00 >= 12 ? cm.getPlayer().getAndroid().getHair() : Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer().getJob()) ? cm.getPlayer().isDressUp() ? cm.getPlayer().getSecondHair() : cm.getPlayer().getHair() : cm.getPlayer().getHair()) / 10) * 10 + i);
                }
                break;
            case 9:
            case 20: // Hair 2
        ...
        if (Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer().getJob())) {
            cm.sendNext("#fs11#Which appearance would you like to style?\r\n\r\n#b#L1#Change appearance before transformation#l\r\n#L2#Change appearance after transformation#l#k");
        } else {
