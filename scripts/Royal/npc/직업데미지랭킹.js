...
function action(mode, type, selection) {
	...
	if (status == 0) {
        var say = "#fs11##b직업별 전투력측정 랭킹입니다.\r\n#k보기 원하시는 직업의 랭킹을 선택해주세요.\r\n";
        ...
        var rs = null;
        var count = 0;
        var say = "#fs11##fc0xFF000000#" + 모험가[selection][1] + " 직업의 전투력측정 랭킹입니다.\r\n#r※ 데미지는 억 단위부터 표기됩니다\r\n\r\n";
        try {
            con = DBConnection.getConnection();
            ps = con.prepareStatement("SELECT `name`, `damage`, `player_id` FROM `damage_measurement_rank` WHERE `job` = " + 모험가[selection][0] + " ORDER BY `damage` DESC LIMIT 100"); //AND `gm` = 0 
            rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                say += "#fc0xFFB2B2B2#" + aas + "#b" + name + "#k | #r데미지 : #e#fc0xFF000000#" + ConvertNumber(rs.getString("damage")) + "#n\r\n";
            }
            rs.close();
            ps.close();
            rsmodify.close();
