...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.getPlayer().getMap().broadcastMessage(Packages.network.models.CField.UIPacket.SetIngameDirectionMode(false, false, false));
        cm.getPlayer().getMap().broadcastMessage(CField.blind(1, 0xff, 0x0, 0x0, 0x0, 0, 0));
        cm.getPlayer().getMap().broadcastMessage(CField.musicChange("Bgm00.img/Silence"));
        statusplus(100);

        Packages.object.utills.Timer.MapTimer.getInstance().schedule(function () {
...
function statusplus(time) {
    cm.getPlayer().getMap().broadcastMessage(Packages.network.models.CField.UIPacket.InGameDirectionEvent("", 1, time));
}
