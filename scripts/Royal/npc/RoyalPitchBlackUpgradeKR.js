...
var itemList = [
   {
      'itemid': 1012643, 'qty': 1, 'allstat': 0, 'atk': 0, // Item ID, quantity, bonus stats, bonus attack (stats apply only to equipment)
      'recipes': [[1012632, 1], [4310237, 5000], [4009005, 2000], [4033172, 500], [4001878, 2000], [4031227, 1000], [4001715, 600]], 'price': 0, 'chance': 50, // Ingredients, cost, probability
      'fail': [[1012632, 1]] // Item given on failure
   },
   {
      'itemid': 1022289, 'qty': 1, 'allstat': 0, 'atk': 0,
      'recipes': [[1022278, 1], [4310237, 5000], [4009005, 2000], [4033172, 500], [4001878, 2000], [4031227, 1000], [4001715, 600]], 'price': 0, 'chance': 50,
      'fail': [[1022278, 1]]
   },
   {
      'itemid': 1032318, 'qty': 1, 'allstat': 0, 'atk': 0,
...
function action(mode, type, sel) {
   ...
   var message = "#fs11#";
   if (status == 0) {
      message += blackColor + "คุณ #h0# จ๊ะ กรุณาเลือกไอเทมที่เธอต้องการจะสร้างหน่อยนะ#fc0xFF990033#\r\n\r\n";
      ...
      if (Packages.objects.utils.Randomizer.rand(1, 100) <= targetItem['chance']) {
         ...
         cm.worldGMMessage(21, "[Pitch Black] คุณ " + cm.getPlayer().getName() + " ได้สร้าง [" + cm.getItemName(targetItem['itemid']) + "] สำเร็จแล้ว!");
      } else {
         cm.sendOk("อุ๊ย... เสริมพลังล้มเหลวซะแล้วจ้ะ...");
         cm.worldGMMessage(21, "[Pitch Black] คุณ " + cm.getPlayer().getName() + " สร้าง [" + cm.getItemName(targetItem['itemid']) + "] ล้มเหลว...");
