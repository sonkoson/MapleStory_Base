/**
 * @projectDescription Cube Script
 *
 * @author vent
 * @version 1.0
 * @sdoc scripts/npc
...
importPackage(Packages.network.game.processors);
importPackage(Packages.objects.item);

var ii = Packages.objects.item.MapleItemInformationProvider.getInstance();
var InventoryHandler = Packages.network.game.processors.inventory.InventoryHandler;
...
var KEEP_LEVEL_PERCENT = 3; // Tier up chance
var enter = "\r\n";
...
function start() {
    /*
      ...
      {
         cm.dispose()
         cm.sendOk('Debugging in progress.')
      }
      */
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        chat += "Hello! I am here to cube your items at Royal Maple!" + enter;
        chat += "There are rules for cubing:" + enter + enter;

        chat += "1. All cubes can only be used on Legendary items with level 100 or higher." + enter;
        chat += "2. Selecting <Before> keeps your current potential and spins again," + enter;
        chat += "Selecting <After> updates the item to the new potential.";

        chat += enter + enter;
        chat += "#L" + CubeStatus.BLACK_CUBE + "# Black Cube" + enter;
        chat += "#L" + CubeStatus.ADDITIONAL_CUBE + "# Additional Cube" + enter;
        chat += "#L" + CubeStatus.RED_CUBE + "# Red Cube" + enter;
        chat += "#L" + CubeStatus.MASTERCRAFT_CUBE + "# Master Craftsman's Cube" + enter;
        //chat += "#L10# Amazing Miracle Cube" + enter;
        ...
        if (!checkCost(cubeState)) {
            chat += "You do not have this Cube.";
            cm.dispose();
        } else {
            switch (selection) {
            ...
            if (cubeItemList.length > 0) {
                chat += "Please select the item you want to cube!" + enter + enter;
                chat += getItemCatalog(cubeItemList);
            } else {
                chat += "There are no items available for cubing.";
                cm.dispose();
            }
        }
        ...
        if (selection == -1) {
            cm.sendSimple("No items meet the conditions.");
            cm.dispose();
        }
        ...
        if (!checkCost(cubeState)) {
            ...
            chat += '#L0#Re-roll' + enter;
            chat += '#L1#Apply New Potential'
        }
        cm.sendOk(chat);
    } else if (status == 3) {
        if (selection == 0) {
            ...
            cm.getPlayer().forceReAddItem(newItem, Packages.objects.item.MapleInventoryType.EQUIP);
            selectedItem = null;
            cm.dispose();
        }
    }
...
function getItemPotential(potentialsData, isAditional) {
    ...
    for (var i = 0; i < potentialsData.length; i++) {
        ...
        chat += "Potential " + (potentialIndex + 1) + ": ";
    ...
    if (!isAditional) {
        ...
        newItem.setPotential2(
            InventoryHandler.potential(
                newItem.getItemId(),
                level == 1 || Packages.objects.utils.Randomizer.nextInt(100) < KEEP_LEVEL_PERCENT ?
                    level :
                    level - 1
            )
        );
...
function getEquipItemList(composer) {
    var equipInventory = cm.getPlayer().getInventory(Packages.objects.item.MapleInventoryType.EQUIP);
