...
function action(mode, type, sel) {
    ...
    if (!cm.getPlayer().isGM() && cm.getPlayer().getDonationPoint() < price && status > 1 && seld != 2) {
        cm.sendOk("#fs11##fc0xFFFF3300#" + price + "P#fc0xFF000000# 강림 후원포인트가 필요합니다.");
        cm.dispose();
        return;
    }
    /*
    ...
    */
    if (status == 0) {
        hpmp = 1000 * n;
        var msg = "#fs11##e#fc0xFFFF3300#[직업변경]#n#fc0xFF000000#\r\n\r\n다른 직업으로 바꿔보시겠어요?\r\n#r※ 제로 직업군과 데몬어벤져 직업군은 직업변경이 불가능합니다.#b" + enter + enter;
        msg += "#L1#직업을 변경하고 싶습니다." + enter;
        msg += "#r#L2#[필독]직업변경에 대해서 궁금합니다." + enter;
        cm.sendSimple(msg);
    } else {
        ...
        switch (seld) {
            case 1:
                if (status == 1) {
                    ...
                    msg += 별 + " #fc0xFF000000##b직업변경 1회#fc0xFF000000#의 가격은 #fc0xFFFF3366#" + price + "P#fc0xFF000000# 강림 후원포인트입니다." + enter;
                    msg += 별 + " 현재 #fc0xFFFF3366##h ##fc0xFF000000# 님의 후원포인트 : #fc0xFFFF3366#" + cm.getPlayer().getDonationPoint() + "P#k" + 검은색 + enter;
                    ...
                    msg += "#fc0xFFFF3366#" + price + " 강림 후원포인트#fc0xFF000000#가 차감되며, 이 시스템에 관련된 복구를 비롯한 모든 문의는 받지 않습니다" + enter;
                    ...
                    if (!cm.getPlayer().isGM() && cm.getPlayer().getDonationPoint() < price) {
                        cm.sendOk("#fs11##b강림 후원포인트#fc0xFF000000#가 부족한 것 같아요");
                        return;
                    }
                    prevjob = cm.getPlayer().getJob();
                    tempjob = jobs[seld2];
                    var oldStat = getJobStatById(prevjob);
                    cm.getPlayer().gainDonationPoint(-price);
                    changeJobscript(tempjob, 1, oldStat);
                    if (deamon == 1) {
...
function arcsymbol(oldType, newType) {
    updateArcSymbolStats(cm.getInventory(-1), MapleInventoryType.EQUIPPED, oldType, newType);
    updateArcSymbolStats(cm.getInventory(1), MapleInventoryType.EQUIP, oldType, newType);
}

function updateArcSymbolStats(inv, invType, oldType, newType) {
    var startSlot = (invType == MapleInventoryType.EQUIPPED) ? -1600 : 0;
    var endSlot = (invType == MapleInventoryType.EQUIPPED) ? -1605 : inv.getSlotLimit();

    for (var i = startSlot; i >= endSlot; i--) {
        var item = inv.getItem(i);
        if (!item || Math.floor(item.getItemId() / 1000) != 1712) continue;

        var arcLevel = item.getArcLevel();
        if (arcLevel <= 0) continue;

        var star = getArcStar(item.getOwner());
        var bonus = 300 * star;

        var curStr = item.getStr();
        var curDex = item.getDex();
        var curInt = item.getInt();
        var curLuk = item.getLuk();

        var isFromXenon = (oldType == 5);
        var isToXenon = (newType == 5);

        if (isToXenon) {
            // 일반 → 제논
            item.setStr(885 + bonus);
            item.setDex(885 + bonus);
            item.setInt(bonus);
            item.setLuk(885 + bonus);
        } else if (isFromXenon) {
            // 제논 → 일반
            var rawStr = Math.max(0, curStr - bonus);
            var rawDex = Math.max(0, curDex - bonus);
            var rawLuk = Math.max(0, curLuk - bonus);

            // 각 스탯에서 885 차감 (총 2655)
            var baseTotal = rawStr + rawDex + rawLuk;
            var totalPenalty = 885 * 3;

            var finalRaw = Math.max(0, baseTotal - totalPenalty);
            var str = 0, dex = 0, intl = 0, luk = 0;

            switch (newType) {
                case 1: str = 2200; break;
                case 2: dex = 2200; break;
                case 3: intl = 2200; break;
                case 4: luk = 2200; break;
            }

            item.setStr(str + bonus);
            item.setDex(dex + bonus);
            item.setInt(intl + bonus);
            item.setLuk(luk + bonus);
        } else {
            // 일반 ↔ 일반
            var rawStr = curStr - bonus;
            var rawDex = curDex - bonus;
            var rawInt = curInt - bonus;
            var rawLuk = curLuk - bonus;

            var stats = [rawStr, rawDex, rawInt, rawLuk];
            var maxVal = Math.max.apply(null, stats);
            if (maxVal < 0) maxVal = 0;

            rawStr = rawDex = rawInt = rawLuk = 0;
            switch (newType) {
                case 1: rawStr = maxVal; break;
                case 2: rawDex = maxVal; break;
                case 3: rawInt = maxVal; break;
                case 4: rawLuk = maxVal; break;
            }

            item.setStr(rawStr + bonus);
            item.setDex(rawDex + bonus);
            item.setInt(rawInt + bonus);
            item.setLuk(rawLuk + bonus);
        }

        cm.getPlayer().forceReAddItem(item, invType);
    }
}

function getArcStar(owner) {
    if (!owner) return 0;
    var m = owner.match(/초월\s*([0-9]+)성/);
...
function changeJobscript(tjob, jt, oldStat) {
    var jid = tjob['jobid'];
    var newStat = tjob['stat'];
