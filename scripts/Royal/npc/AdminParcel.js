...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            var ask = "#fs11#";
            ask += black + "คุณต้องการ #rส่งไอเท็ม" + black + " ประเภทไหน?\r\n\r\n";
            ask += color + "▶ กรุณาเลือกประเภทไอเท็มที่ต้องการส่ง#k\r\n";
            ask += "#L1##b[Equipment]" + black + " ส่งไอเท็ม\r\n";
            ask += "#L2##b[Use]" + black + " ส่งไอเท็ม\r\n";
            ask += "#L3##b[Set-up]" + black + " ส่งไอเท็ม\r\n";
            ask += "#L4##b[Etc]" + black + " ส่งไอเท็ม\r\n";
            ask += "#L5##b[Cash]" + black + " ส่งไอเท็ม\r\n";
            ask += "#L6##b[Cash Equip]" + black + " ส่งไอเท็ม\r\n";
            cm.sendSimple(ask);

        } else if (cm.getPlayer().getReborns() < 0) {
            cm.sendOk("\r\n#bเฉพาะผู้ที่ Rebirth 10 ครั้งขึ้นไปเท่านั้นที่สามารถใช้ได้\r\n");
            cm.dispose();
            ...
            if (selection >= 1 && selection <= 6 || selection == 55) {
                cm.sendGetText("#fs11#" + black + "กรุณากรอกชื่อผู้รับ#r\r\n\r\nผู้รับต้องออนไลน์อยู่ในช่องเดียวกัน หากส่งไอเท็มที่มีอยู่แล้ว ไอเท็มจะหายไป ทีมงานไม่รับผิดชอบต่อความเสียหายจากการใช้งานผิดพลาด");
            } else if (selection == 7) {
                cm.sendOk("#fs11#ระบบส่งพัสดุสามารถส่งไอเท็มทุกประเภทและทุกออปชั่นได้ ต้องใช้ 500M Meso และต้องออนไลน์อยู่ในช่องเดียวกัน ทีมงานไม่รับผิดชอบต่อความเสียหายจากการใช้งานผิดพลาด");
                cm.dispose();
            }
        } else if (status == 2) {
            if (operation == 1) {
            ...
            if (conn == null) {
                cm.sendOk("#fs11#ผู้รับไม่ได้ออนไลน์หรืออยู่คนละช่อง หรือไม่มีชื่อนี้อยู่");
                cm.dispose();
            } else if (!cm.getPlayer().isGM() && !conn.isGM()) {
                cm.sendOk("#fs11#" + color + "สามารถส่งไอเท็มให้ Admin เท่านั้น");
                cm.dispose();
            } else {
                var ok = false;
                var selStr = "#fs11##b" + conn.getName() + black + " ต้องการส่งไอเท็มอะไร?\r\n";
                for (var x = 1; x < 2; x++) {
                        ...
                        if (operation == 55) {
                            for (ii = 0; ii < canitemCube.length; ii++) {
                                if (cm.getInventory(yes).getItem(i).getItemId() == canitemCube[ii]) {
                                    selStr += black + "#L" + (yes * 1000 + i) + "##i" + itemid + "##z" + itemid + "# #r[Slot " + i + "]\#l\r\n";
                                }
                            }
                        } else {
                            selStr += black + "#L" + (yes * 1000 + i) + "##i" + itemid + "##z" + itemid + "# #r[Slot " + i + "]#l\r\n";
                ...
                if (!ok) {
                    cm.sendOk("#fs11#ไม่มีไอเท็มที่จะส่ง");
                    cm.dispose();
                    return;
                }
            ...
            if (statsSel == null) {
                cm.sendOk("#fs11#เกิดข้อผิดพลาด กรุณาแจ้ง Admin");
                cm.dispose();
                return;
            }
            var text = cm.getText();
            ...
            if (conn == null) {
                cm.sendOk("#fs11#ผู้รับออฟไลน์หรือเปลี่ยนช่องแล้ว\r\n\r\n#rยกเลิกการส่งพัสดุ");
                cm.dispose();
                return;
            }
                ...
                if (cm.haveItem(5330000, 0)) {
                    if (Packages.constants.GameConstants.isPet(item.getItemId()) == false) {
                        if (cm.getPlayer().getName() != text) {
                            ...
                            cm.sendOk("#fs11#ส่งพัสดุให้ " + text + " สำเร็จแล้ว");
                            cm.dispose();
                        } else {
                            cm.sendOk("#fs11#ไม่สามารถส่งให้ตัวเองได้");
                            cm.dispose();
                        }
                    } else {
                        cm.sendOk("#fs11#ไม่สามารถส่ง Pet ได้");
                        cm.dispose();
                    }
                } else {
                    cm.sendOk("ไม่มีสิทธิ์ใช้งาน");
                    cm.dispose();
                }
            } else {
                if (!isban) {
            ...
            if (statsSel == null) {
                cm.sendOk("เกิดข้อผิดพลาด กรุณาแจ้ง Admin");
                cm.dispose();
                return;
            }
            ...
            if (item.getQuantity() >= text) {
                if (cm.haveItem(5330000, 0)) {
                    if (cm.getPlayer().getName() != name) {
                        ...
                        cm.sendOk("#fs11#ส่งพัสดุให้ " + name + " สำเร็จแล้ว");
                        cm.dispose();
                    } else {
                        cm.sendOk("ไม่สามารถส่งให้ตัวเองได้");
                        cm.dispose();
                    }
                } else {
                    cm.sendOk("ต้องมี Quick Delivery Ticket เพื่อส่งไอเท็ม");
                    cm.dispose();
                }
            } else {
                cm.sendOk("จำนวนที่กรอกมากกว่าที่มีอยู่");
                cm.dispose();
            }
        }
    }
