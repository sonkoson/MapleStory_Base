...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        talk = "#fs11#이제 욕망의 제단으로 진입하여 진 힐라를 대적해야 한다.\r\n\r\n";
	talk += "#L0##k진 힐라 입장하기#k#b (레벨 265이상)#k#l\r\n";
	//talk += "#L1##b진 힐라 #Cgray#(연습 모드) #k#r(레벨 265이상)#k#l\r\n";
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1])) {
            ...
            talk += "#k#n님의 레벨이 부족합니다.\r\n진 힐라 " + name[st] + "모드는 " + setting[st][3] + " 레벨 이상만 입장 가능합니다.";
            cm.sendOkS(talk, 0x26);
            cm.dispose();
            return;
            ...
            talk += "#k#n님의 보스랭크가 부족합니다.\r\n진 힐라 " + name[st] + "모드는 " + setting[st][4] + " 랭크 이상만 입장 가능합니다.";
            cm.sendOkS(talk, 0x26);
            cm.dispose();
            return;
        } else {
