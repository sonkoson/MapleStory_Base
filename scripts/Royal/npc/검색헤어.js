...
function action(mode, type, selection, dressUp_) {
    ...
    if (status == 0) {
        selection = 0; // 0 헤어 1 성형
        firstSelection = 0; // 0 헤어 1 성형
...
function setAvatar(args) {
    if (isHair(args)) {
        if (Packages.constants.GameConstants.isZero(cm.getPlayer().getJob()) && cm.getPlayer().getZeroInfo().isBeta()) { // 제로 - 베타
