...
var MapleInventoryType = Packages.objects.item.MapleInventoryType;
var Packet = Packages.network.models;
...
var SCRIPT_TITLE = "#e#b<One-Click Rebirth Flame>#n#k";
var enter = "\r\n";
var ALLOWED_REBIRTH_FIRE_ITEM_ID = [2048717]; // Eternal Rebirth Flame
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        chat += "#bMapleStory Convenient One-Click Rebirth Flame System" + enter;
        chat += "#rPlease select the Rebirth Flame you want to use." + enter;
        ...
        if (selectedEquipments.length > 0) {
            chat += "Please select the equipment to apply the Rebirth Flame." + enter;
            ...
            chat += "No items available for enhancement.";
            cm.dispose();
        }
        cm.sendOk(chat);
    } else if (status == 2) {
        ...
        if (selectedEquipment != null && selectedRebirthFireItemId != null) {
            if (checkCost(selectedRebirthFireItemId)) {
                ...
                chat += "#L" + SELECTION_APPLY + "#Use again" + enter;
                chat += "#L" + SELECTION_EXIT + "#Stop using" + enter;
            } else {
                chat += "#v" + selectedRebirthFireItemId + "#";
                chat += "#z" + selectedRebirthFireItemId + "#";
                chat += " is insufficient.";
                cm.dispose();
            }
        } else {
            cm.dispose();
...
function getCalculratedValue(option, value, itemId) {
    ...
    switch (option) {
        ...
        case FireOption.WATTACK:
            if (Packages.constants.GameConstants.isWeapon(itemId)) {
                switch (value) {
                    case 3:
                        if (reqLevel <= 150) {
...
function applyRebirthFire(item, rebirthFireItemId) {
    ...
    cm.getClient().getSession().writeAndFlush(InventoryPacket.updateInventoryItem(false, Packages.objects.item.MapleInventoryType.EQUIP, item));
    cm.getClient().getSession().writeAndFlush(InventoryPacket.updateInventorySlot(Packages.objects.item.MapleInventoryType.EQUIP, item, false));
}
...
function formattedMeso(meso) {
    ...
    if (upperMeso >= 1) mesoString += upperMeso + "00 Million";
    if (lowerMeso > 0) mesoString += lowerMeso + "0,000";
...
function getEquipItems(composer) {
    return getInventoryItems(Packages.objects.item.MapleInventoryType.EQUIP, composer);
}
