...
import objects.users.achievement.caching.mission.submission.checkvalue.ComboKillCount;
import objects.users.achievement.caching.mission.submission.score.Score;

public class ComboKillIncreaseCheck extends AchievementMissionCheck {
   private int comboKill;

   public ComboKillIncreaseCheck(int comboKill) {
      this.comboKill = comboKill;
   }

   @Override
   public boolean check(CheckValue checkValue) {
      boolean comboKillCountCheck = true;
      ComboKillCount comboKillCount = checkValue.getComboKillCount();
      if (comboKillCount != null) {
         comboKillCountCheck = comboKillCount.check(this.comboKill);
      }

      return comboKillCountCheck;
   }

   @Override
   public void checkMission(MapleCharacter player, AchievementSubMissionType type, long delta) {
      int[] ids = new int[]{1382, 1383, 614, 615};
                  ...
                  if (missionEntry != null) {
                     ...
                     if (subMission != null && subMission.getSubMissionType() == type) {
                        ...
                        if (checkValue != null && score != null && this.check(checkValue)) {
                           ...
                           this.updateList.add(AchievementFactory.checkMission(deltaScore, player, achievement, type, mission, score, id, subMissionID));
                        }
                     }
                  }
               }
