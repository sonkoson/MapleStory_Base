...
import objects.fields.fieldset.instance.HardSerenBoss;
import objects.users.MapleCharacter;

public class HardSerenEnter extends FieldSet {
   public HardSerenEnter(int channel) {
      super("HardSerenEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(410002000, 410002020, 410002040, 410002060, 410002080))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 265;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new HardSerenBoss(this, fsim, nCharacter, isPracticeMode);
