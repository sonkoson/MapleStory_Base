...
import objects.fields.fieldset.instance.KaringBoss;
import objects.users.MapleCharacter;

public class NormalKaringEnter extends FieldSet {
   public NormalKaringEnter(int channel) {
      super("NormalKaringEnter", channel);
      this.instanceMaps
         .add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(410007100, 410007120, 410007140, 410007160, 410007180, 410007200, 410007220))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 275;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new KaringBoss(this, fsim, nCharacter, isPracticeMode, "normal");
