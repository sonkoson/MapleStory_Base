...
public class NormalBlackHeavenBossEnter extends FieldSet {
   public NormalBlackHeavenBossEnter(int channel) {
      super("NormalBlackHeavenBossEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(350060700, 350060800, 350060900, 350060950))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 230;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalBlackHeavenBoss(this, fsim, nCharacter, isPracticeMode);
