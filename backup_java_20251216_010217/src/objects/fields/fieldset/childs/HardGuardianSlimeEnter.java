...
import objects.fields.fieldset.instance.HardGuardianSlimeBoss;
import objects.users.MapleCharacter;

public class HardGuardianSlimeEnter extends FieldSet {
   public HardGuardianSlimeEnter(int channel) {
      super("HardGuardianSlimeEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(160010000, 160020000, 160050000))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 210;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new HardGuardianSlimeBoss(this, fsim, nCharacter, isPracticeMode);
