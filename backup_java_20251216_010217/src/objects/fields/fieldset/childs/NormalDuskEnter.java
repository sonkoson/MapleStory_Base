...
public class NormalDuskEnter extends FieldSet {
   public NormalDuskEnter(int channel) {
      super("NormalDuskEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450009400, 450009430))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 245;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalDuskBoss(this, fsim, nCharacter, isPracticeMode);
