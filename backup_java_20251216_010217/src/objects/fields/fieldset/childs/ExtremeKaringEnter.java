...
public class ExtremeKaringEnter extends FieldSet {
   public ExtremeKaringEnter(int channel) {
      super("EasyKaringEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450012210, 450012250))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 255;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalDunkelBoss(this, fsim, nCharacter, isPracticeMode);
