...
public class FlagRaceN1 extends FieldSetInstance {
   boolean isPracticeMode = false;
   boolean startGame = false;
   HashMap<String, Integer> flagGoalNumber = new HashMap<>();

   public FlagRaceN1(FieldSet fs, FieldSetInstanceMap fsim, MapleCharacter leader, boolean isPracticeMode) {
   ...
   public void waitGameStart() {
      this.eventSchedules.put("waitGameStart1", Timer.EventTimer.getInstance().schedule(new Runnable() {
         @Override
         public void run() {
            if (!FlagRaceN1.this.dispose) {
               Field map = FlagRaceN1.this.field(FlagRaceN1.this.fsim.instances.get(0));
         ...
         public void run() {
            if (!FlagRaceN1.this.dispose) {
               ...
               if (map.getCharactersSize() > 0) {
                  ...
                  map.broadcastMessage(FlagRaceN1.this.showFlagRaceSkillUI(true));
               }
            }
         }
      }, 24000L));
      this.eventSchedules.put("waitGameStart4", Timer.EventTimer.getInstance().schedule(new Runnable() {
         ...
         public void run() {
            if (!FlagRaceN1.this.dispose) {
               Field map = FlagRaceN1.this.field(FlagRaceN1.this.fsim.instances.get(0));
               if (map.getCharactersSize() > 0) {
                  FlagRaceN1.this.startGame = true;
