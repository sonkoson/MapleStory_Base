...
public class HardLucidBoss extends FieldSetInstance {
   boolean isPracticeMode = false;
   boolean genesisQuestMode = false;
   private Properties Var = new Properties();

   public HardLucidBoss(FieldSet fs, FieldSetInstanceMap fsim, MapleCharacter leader, boolean isPracticeMode, boolean genesisQuestMode) {
   ...
   public void userEnter(MapleCharacter user) {
      ...
      if (this.genesisQuestMode) {
         if (this.leader.getParty().getPartyMemberList().size() > 1) {
            user.applyBMCurse3();
         }

         if (user.getMap().getId() == 450004450) {
            user.gainItem(2000047, 50);
         }
      }

      if (user.getMap().getId() == 450004450 || user.getMap().getId() == 450004550) {
