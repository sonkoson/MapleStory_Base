...
public class ItemLog extends LoggingEntry {
   private int itemID;
   ...
   public ItemLog(MapleCharacter player, int channel, int itemID, int quantity, int fieldID, int type, long price, long serialNumber, StringBuilder log) {
      super(LoggingType.ItemLog, player.getClient().getAccountName(), player.getName(), player.getId(), player.getAccountID(), log, System.currentTimeMillis());
