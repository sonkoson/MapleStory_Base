...
public class HellDunkelEnter extends FieldSet {
   public HellDunkelEnter(int channel) {
      super("HellDunkelEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450012600, 450012650))));
      this.minMember = 1;
      this.maxMember = 3;
      this.minLv = 280;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new HellDunkelBoss(this, fsim, nCharacter, isPracticeMode);
