...
public class PuzzleMasterEnter extends FieldSet {
   public PuzzleMasterEnter(int channel) {
      super("PuzzleMasterEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(993194800, 993194900))));
      this.minMember = 1;
      this.maxMember = 3;
      this.minLv = 200;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new PuzzleMaster(this, fsim, nCharacter);
