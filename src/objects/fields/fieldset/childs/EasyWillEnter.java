...
public class EasyWillEnter extends FieldSet {
   public EasyWillEnter(int channel) {
      super("EasyWillEnter", channel);
      this.instanceMaps
         .add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450007800, 450007850, 450007900, 450007950, 450008000, 450008050, 450008080))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 235;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new EasyWillBoss(this, fsim, nCharacter, isPracticeMode);
