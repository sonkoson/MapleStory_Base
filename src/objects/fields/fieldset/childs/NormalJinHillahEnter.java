...
public class NormalJinHillahEnter extends FieldSet {
   public NormalJinHillahEnter(int channel) {
      super("NormalJinHillahEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450010900, 450010930, 450010950))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 250;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalJinHillahBoss(this, fsim, nCharacter, isPracticeMode);
