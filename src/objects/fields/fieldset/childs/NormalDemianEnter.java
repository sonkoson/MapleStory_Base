...
public class NormalDemianEnter extends FieldSet {
   public NormalDemianEnter(int channel) {
      super("NormalDemianEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(350160260, 350160200, 350160220, 350160240, 350160280))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 200;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalDemianBoss(this, fsim, nCharacter, isPracticeMode);
