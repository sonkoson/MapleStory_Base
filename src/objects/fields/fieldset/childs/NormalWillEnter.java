...
public class NormalWillEnter extends FieldSet {
   public NormalWillEnter(int channel) {
      super("NormalWillEnter", channel);
      this.instanceMaps
         .add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450008700, 450008750, 450008800, 450008850, 450008900, 450008950, 450008980))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 235;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalWillBoss(this, fsim, nCharacter, isPracticeMode);
