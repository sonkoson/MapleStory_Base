...
public class NormalLucidEnter extends FieldSet {
   public NormalLucidEnter(int channel) {
      super("NormalLucidEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450004100, 450004150, 450004200, 450004250, 450004300))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 220;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalLucidBoss(this, fsim, nCharacter, isPracticeMode);
