...
public class ChaosDuskEnter extends FieldSet {
   public ChaosDuskEnter(int channel) {
      super("ChaosDuskEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450009450, 450009480))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 245;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new ChaosDuskBoss(this, fsim, nCharacter, isPracticeMode);
