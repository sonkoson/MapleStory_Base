...
public class NormalGuardianSlimeEnter extends FieldSet {
   public NormalGuardianSlimeEnter(int channel) {
      super("NormalGuardianSlimeEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(160030000, 160040000, 160060000))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 210;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            ...
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new NormalGuardianSlimeBoss(this, fsim, nCharacter, isPracticeMode);
