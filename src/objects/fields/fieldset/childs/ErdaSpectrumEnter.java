...
public class ErdaSpectrumEnter extends FieldSet {
   public ErdaSpectrumEnter(int channel) {
      super("ErdaSpectrumEnter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(450001400, 450001450, 450001500))));
      this.minMember = 1;
      this.maxMember = 3;
      this.minLv = 200;
      this.maxLv = 1000;
      ...
      synchronized (this.fInstance) {
         ...
         if (enterInteger > 6 && nCharacter != null) {
            for (MapleCharacter c : nCharacter.getMap().getCharacters()) {
               if (c.getParty() != null && c.getParty().getId() == nCharacter.getParty().getId() && c.getOneInfoQuestInteger(34170, "count") >= 3) {
                  return -2;
               }
            }
         }
         ...
         if (enterInteger > 6 && nCharacter != null) {
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new ErdaSpectrum(this, fsim, nCharacter);
