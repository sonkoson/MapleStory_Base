...
public class FlagRaceN1Enter extends FieldSet {
   public FlagRaceN1Enter(int channel) {
      super("FlagRaceN1Enter", channel);
      this.instanceMaps.add(new FieldSetInstanceMap(new ArrayList<>(Arrays.asList(942003100))));
      this.minMember = 1;
      this.maxMember = 6;
      this.minLv = 101;
      this.maxLv = 1000;
         ...
         if (enterInteger > 6 && nCharacter != null) {
            for (FieldSetInstanceMap fsim : this.instanceMaps) {
               if (fsim.instances.get(0) == enterInteger) {
                  FieldSetInstance fsi = new FlagRaceN1(this, fsim, nCharacter, isPracticeMode);
