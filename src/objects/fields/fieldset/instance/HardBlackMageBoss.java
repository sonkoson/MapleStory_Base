...
public class HardBlackMageBoss extends FieldSetInstance {
   boolean isPracticeMode = false;
   private Properties Var = new Properties();

   public HardBlackMageBoss(FieldSet fs, FieldSetInstanceMap fsim, MapleCharacter leader, boolean isPracticeMode) {
   ...
   public void userEnter(MapleCharacter user) {
      ...
      } else if (user.getMap().getId() == 450013000) {
         user.gainItem(4001895, -1, false, -1L, "");
      }
