...
public class MapleFitness extends MapleEvent {
   ...
   public void checkAndMessage() {
      this.msgSchedule = Timer.EventTimer.getInstance()
         .register(
            new Runnable() {
               @Override
               public void run() {
                  ...
                  if (timeLeft > 9000L && timeLeft < 11000L) {
                     MapleFitness.this.broadcast(
                        CWvsContext.serverNotice(
                           0, "You have 10 sec left. Those of you unable to beat the game, we hope you beat it next time! Great job everyone!! See you later~"
                        )
                     );
                     ...
                     MapleFitness.this.broadcast(
                        CWvsContext.serverNotice(
                           0, "For those who have heavy lags, please make sure to move slowly to avoid falling all the way down because of lags."
                        )
                     );
                     ...
                     MapleFitness.this.broadcast(
                        CWvsContext.serverNotice(
                           0, "The 2nd stage offers monkeys throwing bananas. Please make sure to avoid them by moving along at just the right timing."
                        )
                     );
