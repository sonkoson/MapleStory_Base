...
public class Event_DojoAgent {
   ...
   public static boolean warpStartAgent(MapleCharacter c, boolean party) {
      ...
      for (int i = 970030100; i < 970030115; i++) {
         Field map = ch.getMapFactory().getMap(i);
   ...
   public static boolean warpStartDojo(MapleCharacter c, boolean party) {
      ...
      for (int x = 0; x < 10; x++) {
         ...
         for (int i = 1; i < 39; i++) {
            Field map = ch.getMapFactory().getMap(925020000 + 100 * i + x);
            if (map.getCharactersSize() > 0) {
               canenterr = false;
               break;
            }
      ...
      Field map = ch.getMapFactory().getMap(mapid);
      Field mapidd = c.getMap();
      if (canenter) {
         if (party && c.getParty() != null) {
            for (PartyMemberEntry mem : c.getParty().getPartyMemberList()) {
   ...
   public static void failed(MapleCharacter c) {
      Field currentmap = c.getMap();
      Field deadMap = c.getClient().getChannelServer().getMapFactory().getMap(925020002);
   ...
   public static boolean warpNextMap(MapleCharacter c, boolean fromResting, Field currentmap) {
      try {
         int temp = (currentmap.getId() - 925000000) / 100;
         int thisStage = temp - temp / 100 * 100;
         int points = getDojoPoints(thisStage);
         GameServer ch = c.getClient().getChannelServer();
         Field deadMap = ch.getMapFactory().getMap(925020002);
         if (!c.isAlive()) {
            c.changeMap(deadMap, deadMap.getPortal(0));
            return true;
         }

         Field map = ch.getMapFactory().getMap(currentmap.getId() + 100);
            ...
            if (c.getParty() != null && c.getParty().getPartyMemberList().size() > 1) {
               for (PartyMemberEntry mem : c.getParty().getPartyMemberList()) {
                  ...
                  if (chr != null) {
                     ...
                     chr.getQuestIfNullAdd(MapleQuest.getInstance(150100)).setCustomData(String.valueOf(dojo));
               ...
               c.getQuestIfNullAdd(MapleQuest.getInstance(150100)).setCustomData(String.valueOf(dojo));
         ...
         if (currentmap.getId() >= 925023800 && currentmap.getId() <= 925023814) {
            Field lastMap = ch.getMapFactory().getMap(925020003);
            if (c.getParty() != null && c.getParty().getPartyMemberList().size() > 1) {
               for (PartyMemberEntry memx : c.getParty().getPartyMemberList()) {
                  MapleCharacter chr = currentmap.getCharacterById(memx.getId());
                  if (chr != null) {
                     if (!chr.isAlive()) {
                        chr.addHP(50L);
                     }
         ...
         if (map != null && map.getCharactersSize() == 0) {
            clearMap(map, false);
            if (c.getParty() != null) {
               for (PartyMemberEntry memxx : c.getParty().getPartyMemberList()) {
                  MapleCharacter chr = currentmap.getCharacterById(memxx.getId());
                  if (chr != null) {
                     if (!chr.isAlive()) {
                        chr.addHP(50L);
                     }
         ...
         if (map != null) {
            ...
            for (int x = 0; x < 10; x++) {
               Field mapz = ch.getMapFactory().getMap(basemap + x);
               if (mapz.getCharactersSize() == 0) {
                  clearMap(mapz, false);
                  if (c.getParty() != null) {
                     for (PartyMemberEntry memxxx : c.getParty().getPartyMemberList()) {
                        MapleCharacter chr = currentmap.getCharacterById(memxxx.getId());
                        if (chr != null) {
                           if (!chr.isAlive()) {
                              chr.addHP(50L);
                           }
         ...
         Field mappz = ch.getMapFactory().getMap(925020001);
         if (c.getParty() != null) {
            for (PartyMemberEntry memxxxx : c.getParty().getPartyMemberList()) {
               MapleCharacter chr = currentmap.getCharacterById(memxxxx.getId());
               if (chr != null) {
   ...
   private static final void spawnMonster(final Field map, int stage) {
      final int mobid;
      switch (stage) {
         case 1:
            mobid = 9300184;
               ...
               new Runnable() {
                  ...
                  public void run() {
                     map.spawnMonsterWithEffect(
                        ...
                        15,
                        rand == 0 ? Event_DojoAgent.point1 : (rand == 1 ? Event_DojoAgent.point2 : Event_DojoAgent.point3)
                     );
                  }
               },
               3000L
