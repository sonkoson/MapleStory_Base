...
public class Field_FerociousBattlefield extends Field {
         ...
         for (MapleCharacter p : this.getCharactersThreadsafe()) {
            if (p.getParty() != null) {
               if (p.getMapId() == this.getId()) {
                  int quantity = 14;
                  p.gainItem(4001893, (short)quantity, false, -1L, "Item obtained from defeating Dusk");
               }
               ...
               if (!set) {
                  ...
                  if (mob.getId() == 8950117) {
                     ...
                     for (PartyMemberEntry mpc : new ArrayList<>(p.getParty().getPartyMemberList())) {
                        ...
                        StringBuilder sb = new StringBuilder("Boss Chaos Dusk Defeated");
                     ...
                     for (PartyMemberEntry mpcx : new ArrayList<>(p.getParty().getPartyMember().getPartyMemberList())) {
                        StringBuilder sb = new StringBuilder("Boss Normal Dusk Defeated");
   ...
   public void doTentaclesPattern() {
      ...
      this.sendDuskNotice("หน้าที่ป้องกันอยู่จะโจมตีอย่างรุนแรง! ถ้ากันได้ จะสามารถโจมตีดวงตาแห่งความว่างเปล่าที่เปิดออกได้!", 3000);
