...
public class RankingWorker {
   private static final Map<Integer, List<RankingWorker.RankingInformation>> rankings = new HashMap<>();
   ...
   public static final void run() {
      System.out.println("Updated Characters' Ranking.");
      ...
      try (Connection con = DBConnection.getConnection()) {
         ...
         System.out.println("RankWorker Err");
         var8.printStackTrace();
         FileoutputUtil.outputFileError("Log_Script_Except.rtf", var8);
         System.err.println("Could not update rankings");
      }

      System.out.println("Done loading Rankings in " + (System.currentTimeMillis() - startTime) / 1000L + " seconds :::");
   }
   ...
   private static void updateRanking(Connection con) throws Exception {
      ...
      while (rs.next()) {
         ...
         if (rankMap.containsKey(job / 100)) {
            ...
            rankings.get(-1)
               .add(new RankingWorker.RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), ++rank, rs.getInt("fame")));
            rankings.get(job / 100)
               .add(new RankingWorker.RankingInformation(rs.getString("name"), job, rs.getInt("level"), rs.getInt("exp"), jobRank, rs.getInt("fame")));
            ps.setInt(1, jobRank);
