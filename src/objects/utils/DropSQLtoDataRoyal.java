...
public class DropSQLtoDataRoyal {
   public static void main(String[] args) {
      ...
      try (Connection con = DBConnection.getConnection()) {
         ps = con.prepareStatement("SELECT * FROM drop_data_2");
         rs = ps.executeQuery();
         ...
         while (rs.next()) {
            ...
            list.add(
               new MonsterDropEntry(
                  itemid, chance, rs.getInt("minimum_quantity"), rs.getInt("maximum_quantity"), rs.getInt("questid"), dropperID, rs.getInt("private") == 1
               )
            );
      ...
      for (Entry<Integer, List<MonsterDropEntry>> entry : ret.entrySet()) {
         try {
            ...
            try {
               newTable.save(Paths.get("./data/Ganglim/Drops/" + entry.getKey() + ".data"));
      ...
      try (Connection con = DBConnection.getConnection()) {
         ps = con.prepareStatement("SELECT * FROM drop_data_2_global");
         rs = ps.executeQuery();
      ...
      try {
         newTable.save(Paths.get("./data/Ganglim/Drops/Global/Global.data"));
