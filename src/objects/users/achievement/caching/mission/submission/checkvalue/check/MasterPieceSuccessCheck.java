...
import objects.users.achievement.caching.mission.submission.checkvalue.ItemReward;
import objects.users.achievement.caching.mission.submission.score.Score;

public class MasterPieceSuccessCheck extends AchievementMissionCheck {
   int labelGrade;

   public MasterPieceSuccessCheck(int... value) {
      this.labelGrade = value[0];
   }

   @Override
   public boolean check(CheckValue checkValue) {
      boolean labelCheck = true;
      ItemReward itemRewardCheck = checkValue.getItemReward();
      if (itemRewardCheck != null) {
         labelCheck = itemRewardCheck.check(this.labelGrade);
      }
   ...
   public void checkMission(MapleCharacter player, AchievementSubMissionType type, long delta) {
      int[] ids = new int[]{21, 22, 23, 24, 25, 26, 27};
