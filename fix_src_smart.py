...
def smart_decode(content):
    ...
    i = 0
    while i < len(content):
        # Try to form a UTF-8 sequence starting at i
        c = content[i]
        b = char_to_byte(c)
        
        # Optimization: ASCII is pass-through
        if b != -1 and b < 0x80:
            res.append(c) # Keep as char
            i += 1
            continue
            
        # Possible start of UTF-8 sequence
        if b != -1 and (b >= 0xC0):
            # Try to consume following bytes
            ...
            if i + needed < len(content):
                ...
                for k in range(needed):
                    ...
                    # Handle Space/Holes
                    if next_b == 0x20:
                         # Try replacing with 0x81, 0x8D, 0x8F, 0x90, 0x9D
                         # Heuristic: 0x81 (ก) is most common
                         # Check if 0x81 valid
                         if (0x81 & 0xC0) == 0x80: # It is 10xxxxxx
                             next_b = 0x81
...
def fix_file(filepath):
    ...
    fixed = smart_decode(content)
    
    if fixed != content:
        # Sanity: if we created "กิลด์" or "คุณ"
        if "กิลด์" in fixed or "คุณ" in fixed or "ได้รับ" in fixed or "ไม่สามารถ" in fixed:
